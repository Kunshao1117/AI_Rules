...
- Team-First routing applies before tool selection: use `formal-readonly` for commit scan, changelog-quality evidence, review, validation, release-readiness, memory/docs, source/doc evidence, and deep-read stations; use GO-backed `formal-write` only for captain-owned approved changelog or protected commit actions. Workflow name is route only; it does not authorize writes, git, release, memory mutation, or unbounded specialist launch.

...
- **Enter Plan Mode** (「宣告進入規劃階段：在對話中列出完整實作步驟後再執行」). Use 「對話任務清單」 to track implementation steps.
...
- Call 「規劃完成，開始執行階段」. Begin writing source code using `Write`/`Edit` tools.
- Apply `[SEC SILENT GATE]` before each file write (see `code-quality` rule).
- Mark each 「對話任務清單」 item `completed` as you finish it.

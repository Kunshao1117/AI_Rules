...
1. Captain entry minimum: start with workflow row `06`, the route summary below, `.agents/shared/workflow-capability-evidence-matrix.md` row `06`, `.agents/shared/policies/workflow-orchestration.md` for route/authorization order, and the minimum Team-Native entry gate in `.agents/shared/policies/team-native-core.md`.
...
6. Skill/stage governance: read `.agents/shared/skill-governance.md` only when editing workflow entries, skills, shared policies, or governance boundaries; read `.agents/shared/policies/verification-strategy.md` when selecting evidence, tests, failure routing, or review rendering; read `.agents/shared/workflow-stage-procedures.md` only when the concrete phase checklist is needed, using section `06 Verify` without copying it back here.
...
- Route summary: Select minimum-sufficient verification evidence, define the target, command or operator path, pass/fail state, and route classified failures without repairing.
- Direct Verify records its bounded evidence outcome without a Team board, station, or formal trace. Delegated routes load Team artifacts only after delegated topology resolves.
- A failure does not automatically launch `deep-audit`; use it only for the positive triggers in `verification-strategy.md`.

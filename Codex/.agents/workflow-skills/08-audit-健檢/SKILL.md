---
name: "08-audit-健檢"
description: "Use when: 全光譜專案健檢、audit、證據式健檢、專案型態偵測、相容性檢查、治理巡檢、基礎盤點、深度邏輯審查、真實驗證、plugin、VSIX、Release、version、tag、update reminder 與健康報告。DO NOT use when: 只要單一測試或單一 bug 修復。"
metadata:
  author: antigravity
  version: "2.0"
  origin: framework
...
> 本工作流是健檢總控入口。它保留既有三階段健檢語義，但新增「專案型態偵測 → 動態掛載模組 → 證據式報告」內核。
...
├── Step 1: Run profile detection through Phase 1.
├── Step 2: Run baseline, governance, and compatibility checks through Phase 1.
├── Step 3: Run semantic, security, API/data-flow, and real-evidence checks through Phase 2.
├── Step 4: Run evidence-only checks through Phase 2 when requested.
├── Step 5: Merge evidence packets and emit the final report through Phase 3.

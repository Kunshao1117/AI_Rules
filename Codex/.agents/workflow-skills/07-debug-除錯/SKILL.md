---
name: "07-debug-除錯"
description: "除錯定位與影響說明：適用於分析 stack trace、閱讀日誌、定位錯誤來源並說明商業影響（Use when: debug, logs, stack trace）。不適用於已決定修改原始碼的修復工作（DO NOT use when: source fix）。"
metadata:
  author: antigravity
  version: "2.0"
  origin: framework
...
It selects workflow row `07`, applies the platform adapter, and points to shared procedures when details are needed.
...
1. Captain entry minimum: start with workflow row `07`, the route summary below, `.agents/shared/workflow-capability-evidence-matrix.md` row `07`, `.agents/shared/policies/workflow-orchestration.md` for route/authorization order, and the minimum Team-Native entry gate in `.agents/shared/policies/team-native-core.md`.
...
6. Skill/stage governance: read `.agents/shared/skill-governance.md` only when editing workflow entries, skills, shared policies, or governance boundaries; read `.agents/shared/workflow-stage-procedures.md` only when the concrete phase checklist is needed, using section `07 Debug` without copying it back here.
...
- Repeated hypothesis failure: use `loop_control.transition_decision`; use `structured-reasoning` when multiple competing causes or architecture-level trade-offs remain.

...
- **Enter Plan Mode** (「宣告進入規劃階段：在對話中列出完整實作步驟後再執行」). Use 「對話任務清單」 to track fix steps.
...
- [HALT] STAGE 1 has NO permission to write source files. 「規劃完成，開始執行階段」 is NOT called here.
...
- Call 「規劃完成，開始執行階段」. Apply fixes using `Write`/`Edit` tools.

...
description: "Use when: 健檢第一階段、專案型態偵測、平台能力快照、基礎盤點、依賴安全掃描、記憶卡拓樸、技能覆蓋率、相容性與目錄衛生檢查。DO NOT use when: 要完整健檢入口，改用 08-audit。"
metadata:
  author: antigravity
  version: "2.0"
  origin: framework
...
# [SKILL: /08_audit — Phase 1: 型態偵測、基礎盤點與治理拓樸]

> 本工作流由 `08-audit-健檢` 入口觸發。它產出 project profile、baseline、governance、compatibility evidence packets.
...
## 1.2 Project Surface And Capability Profile
...
## 1.3 Deterministic Baseline Scan
...
If a command is not available, create an `unverified` evidence packet instead of treating the check as passing.

## 1.4 Governance Topology Check
...
## 1.5 Compatibility And Directory Hygiene

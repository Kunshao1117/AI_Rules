...
- Optional alignment route: when ambiguity or risk signals exist, use the existing intent envelope, overreach check, and `drift_check` before `execution_spec` or build-plan; retries use `loop_control.transition_decision`. None of these authorizes writes.

...
2. **技能孤島** — 每個 Codex 專案各自維護技能 → 61 套共用操作型技能一次部署，統一真實來源
...
| **Team-Native Core 團隊原生核心** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入 Team-Native Core；總監只對隊長說話。隊長制不是單人主線加可選子代理，而是站點優先的團隊狀態機：隊長只負責調度、整合、裁決與總監溝通；隊員分別負責實作變更交付、記憶交付、驗證、審查、完成閘門等單一任務。隊長先做草案包，GO 後建立正式派工板，再依波次派隊員；下一波必須以上一波回收證據與明確啟動條件為輸入。草案證據只能支持轉正式派工，不得冒充正式驗收。Codex 可把站點路由到 native subagents、專案自訂代理、瀏覽器、終端、背景任務或 MCP 證據；每站仍要標示 native、adapter、conditional 或 unavailable。正式團隊完成必須回收 implementation change delivery、memory delivery、review、validation 四類交付件與 Team-Native trace；隊長最後只整合已回收且可追溯的變更交付件與證據，不能讓隊員取代主代理寫檔、審查狀態或交付裁決 |
| **範圍式授權解析** | Codex approval/sandbox 提示、工具確認、GO 與 `$skill-name` 都必須收斂到目前明示的計畫、站點、命令、工具或檔案集合；skill 只做工作流路由，介面同意可作為該提示範圍的授權證據，但不是無範圍寫入或記憶/git/release 授權 |
...
graph TB
    ...
    subgraph "目標專案（部署後）"
        ...
        SKILLS[".agents/skills/<br/>78 套（61 共用 + 17 工作流）"]
...
負責將 `.codex/` 治理規則與 78 套技能部署到目標專案。所有 PowerShell 程式碼均配備完整的繁體中文行內說明，三個平台部署能力完全對等。

#### 兩種部署模式

| 模式 | 觸發條件 | 行為 |
|------|---------|------|
| **Fresh** | 專案無 `.codex/` 目錄 | D06 安全網備份記憶 → 部署 `.codex/` 治理規則 → 注入 61 套共用技能 → 合併 17 套工作流技能 → 建立基礎設施 → 寫入版本檔 → 還原記憶 |
| **Upgrade** | 專案已有 `.codex/` 目錄 | 掃描 `.codex/` 差異 → 彩色報告 → 顯示 CHANGELOG → 確認閘門 → 套用 `.codex/` 變更 → 差異注入技能更新 |

#### 技能部署兩步驟

```
Step 1: Shared/skills/ → .agents/skills/    （61 套共用技能）
...
#### 共用技能（61 套）—— 源自 `Shared/skills/`

與 Antigravity 和 Claude Edition 完全同步，涵蓋記憶操作、品質約束、測試策略、MCP 操作食譜、代碼知識圖譜等完整分類。新版新增 team-specialist-registry 與十個 team-specialist-* 專家子技能，並保留 team-role-boundaries、team-change-delivery-artifact、team-memory-docs-delivery-artifact、team-validation-delivery-artifact、team-review-delivery-artifact、team-completion-gate 作為團隊交付技能；專家分類、角色邊界、實作變更交付、記憶交付、驗證、審查與完成閘門拆成單一責任。詳見 [Antigravity/README.md](../Antigravity/README.md) 的技能表格。
...
| `03-build-建構/` | `$03-build-建構` | 設計到建構合約：治理深度判定 + 架構判斷 + 建構計畫 → GO → 正式派工板 → 第 1 波變更交付件 → 回收證據 → 達到下一波啟動條件後再派審查/驗證交付件 → 隊長整合 → 記憶歸卡 |
| `03-1-experiment-實驗/` | `$03-1-experiment-實驗` | 沙盒快速實驗（保留最小團隊站點宣告） |
| `04-fix-修復/` | `$04-fix-修復` | 兩階段修復：診斷 → GO → 正式派工板 → 第 1 波修復變更交付件 → 回收證據 → 達到下一波啟動條件後再派審查/回歸驗證交付件 → 隊長整合 → 記憶更新 |
...
| `09-commit-紀錄總結/` | `$09-commit-紀錄總結` | 授權備份：掃描 → CHANGELOG 草稿 → GO → 正式派工板 → 審查/收尾證據交付件分波回收 → 隊長更新變更紀錄 + 明確清單遠端備份 |
...
Codex 健檢入口採深層證據式架構：先選擇快速、標準、深度或鑑識模式，再偵測網站、後端、命令列、桌面、外掛、函式庫、基礎設施、資料管線、AI 功能、文件治理庫或混合專案，並建立功能、端點、命令、任務、介面、資料流、效能與風險盤點。Codex 版優先使用技能漸進載入、沙盒/審批轉錄、站點要求的唯讀子代理、瀏覽器、終端、MCP 與雲端背景任務採證；中繼證據只允許寫入健檢日誌，不授權修改原始碼、記憶卡或外部狀態。

08 以外的工作流共用外部接地矩陣：每個工作流技能都對應任務型態、官方或標準依據、最低證據狀態、阻塞條件與下一流程路由。Codex 版把矩陣轉譯成技能漸進載入、沙盒/審批紀錄、終端、瀏覽器、MCP、背景任務與站點要求的唯讀子代理證據；工作流技能本身只路由，不授權寫入。若必要證據分支不可用，必須標記未驗證、阻塞或具體主線直做例外。
...
| **子代理人** | 隊長任務板建立後 → Gemini CLI / `@` 指派 / browser-capable agent / Antigravity plugin adapter / 受治理隔離變更交付 | 隊長任務板建立後 → description-driven subagent / `@agent` / governed `Agent(...)` / 受治理隔離變更交付 | 隊長任務板建立後 → Codex native subagents 或 `.codex/agents/*.toml` / 受治理隔離變更交付；Director 要求只會強制建板派工，不允許先開代理；不可用時標記未驗證、阻塞或總監風險關閉但非完整 |
...
| **技能來源** | Shared/ 61 套 | Shared/ 61 套 | Shared/ 61 套 + workflow-skills/ 17 套 |
| **技能總數** | 61 套 | 61 套 | **78 套** |
...
└── .agents/
    ├── skills/                    ← 78 套技能（61 共用 + 17 工作流，扁平結構）
    │   ├── _index.md              ← 技能路由表
    │   ├── memory-ops/            ← 記憶操作指引
    │   ├── code-quality/          ← 品質約束
    │   ├── 00-chat-聊天/          ← 工作流技能（對話）

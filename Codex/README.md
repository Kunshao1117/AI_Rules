...
| `08-audit-健檢/` | `$08-audit-健檢` | 專案型態偵測、平台能力快照、動態掛載模組與證據式健康報告 |
| `08-1-infra-基礎盤點/` | `$08-1-infra-基礎盤點` | 專案型態、基礎盤點、治理拓樸與相容性證據 |
| `08-2-logic-深度邏輯/` | `$08-2-logic-深度邏輯` | 安全/API/資料流語意審查與真實操作證據 |
| `08-3-report-健檢總結/` | `$08-3-report-健檢總結` | 證據等級、燈號、未驗證/阻塞與修復路由報告 |
...
記憶卡是來源記憶，不是可執行技能。目標標準主檔為 `MEMORY.md`；既有專案仍可能在相容期保留舊 `SKILL.md`，正式更名必須透過管理器的記憶主檔遷移入口乾跑、確認 cartridge-system 支援、檢查雙主檔衝突後另行授權套用。本專案目前尚未執行實體記憶卡主檔更名。

乾跑入口：`.\Scripts\AI-RulesManager.ps1 -Action MemoryMigration -Target .`。正式套用更名需另一次明確授權並加上 `-Apply`；在外部記憶引擎支援新主檔前，不應對正式專案執行。
...
UI 驗收使用「介面適配與視覺證據」矩陣：網頁介面收集手機、平板、桌面 viewport；桌面 GUI 收集最小視窗、縮放、高 DPI、對話框與鍵盤操作；外掛面板收集窄側欄、展開寬度、明暗主題與確認狀態；終端工具收集輸出換行、錯誤可讀性、失敗碼與非互動模式。測試入口依治理深度選擇最低證據、加強證據或明確豁免，證據類型必須與介面類型一致。

Codex Edition 同步套用跨專案真實驗證契約：能啟動、操作、呼叫、查詢、截圖、讀日誌或觀察副作用時，AI 必須實測。mock、fixture、假資料、靜態截圖或局部單元測試只能作為局部證據；若功能依賴真實資料、執行期狀態、持久化、外部整合、命令輸出、自動化、雲端服務或操作者可見行為，缺少真實執行證據即不得宣稱完成。Codex 可用瀏覽器、桌面操作、終端、MCP、外掛宿主或 preview/deployment 工具時，必須先搜尋可用入口、確認就緒、重試短暫失敗，或改用等價真實路徑；不能因一次工具不可用就放棄該驗證方式。

Codex 健檢入口採全光譜證據式架構：先偵測網站、後端、命令列、桌面、外掛、函式庫、基礎設施、資料管線、AI 功能、文件治理庫或混合專案，再依平台能力掛載基礎、治理、邏輯、真實操作、效能、發布與相容性檢查。Codex 版優先使用技能漸進載入、沙盒/審批轉錄、明確允許的子代理、瀏覽器、終端、MCP 與雲端背景任務採證；中繼證據只允許寫入健檢日誌，不授權修改原始碼、記憶卡或外部狀態。

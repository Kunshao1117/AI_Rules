...
2. **技能孤島** — 每個 Codex 專案各自維護技能 → 43 套共用操作型技能一次部署，統一真實來源
...
| **隊長制編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入 captain-led mode；每站必須標示是否適用、執行模式、證據負責人、角色邊界、主線直做例外與完成條件；證據型站點預設使用 Codex native subagents 或等價唯讀證據分支，實作隊員只能在受治理隔離區產出 patch packet，不能取代主代理寫檔、審查狀態或交付裁決 |
...
graph TB
    ...
    subgraph "目標專案（部署後）"
        ...
        SKILLS[".agents/skills/<br/>60 套（43 共用 + 17 工作流）"]
...
負責將 `.codex/` 治理規則與 60 套技能部署到目標專案。所有 PowerShell 程式碼均配備完整的繁體中文行內說明，三個平台部署能力完全對等。

#### 兩種部署模式

| 模式 | 觸發條件 | 行為 |
|------|---------|------|
| **Fresh** | 專案無 `.codex/` 目錄 | D06 安全網備份記憶 → 部署 `.codex/` 治理規則 → 注入 43 套共用技能 → 合併 17 套工作流技能 → 建立基礎設施 → 寫入版本檔 → 還原記憶 |
| **Upgrade** | 專案已有 `.codex/` 目錄 | 掃描 `.codex/` 差異 → 彩色報告 → 顯示 CHANGELOG → 確認閘門 → 套用 `.codex/` 變更 → 差異注入技能更新 |

#### 技能部署兩步驟

```
Step 1: Shared/skills/ → .agents/skills/    （43 套共用技能）
...
| **Captain-led programming governance coverage** | Doctor 檢查隊長制編程治理共用技能、隊長任務板、角色邊界、隔離補丁、證據負責人、主線直做例外、全主線假團隊防線、00/01 自動轉向、三平台 workflow 接入與部署後 shared skill / shared reference hash 是否一致 |
...
| **子代理人** | 隊長站點 → Gemini CLI / `@` 指派 / browser-capable agent / Antigravity plugin adapter / 受治理隔離補丁 | 隊長站點 → description-driven subagent / `@agent` / governed `Agent(...)` / 受治理隔離補丁 | 隊長站點 → Director 要求、工作流站點要求或 `.codex/agents/*.toml` / 受治理隔離補丁；不可用時標記未驗證、阻塞或主線直做例外 |
...
| **技能來源** | Shared/ 43 套 | Shared/ 43 套 | Shared/ 43 套 + workflow-skills/ 17 套 |
| **技能總數** | 43 套 | 43 套 | **60 套** |
...
└── .agents/
    ├── skills/                    ← 60 套技能（43 共用 + 17 工作流，扁平結構）
    │   ├── _index.md              ← 技能路由表
    │   ├── memory-ops/            ← 記憶操作指引
    │   ├── code-quality/          ← 品質約束
    │   ├── 00-chat-聊天/          ← 工作流技能（對話）

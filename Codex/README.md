...
graph TB
    ...
    subgraph "目標專案（部署後）"
        ...
        SKILLS[".agents/skills/<br/>79 套（62 共用 + 17 工作流）"]
...
| **子代理人** | 隊長任務板建立後 → Gemini CLI / `@` 指派 / browser-capable agent / Antigravity plugin adapter / 受治理隔離變更交付 | 隊長任務板建立後 → description-driven subagent / `@agent` / governed `Agent(...)` / 受治理隔離變更交付 | 隊長任務板建立後 → Codex native subagents 或專案自訂代理 / 受治理隔離變更交付；Director 要求只會強制建板派工，不允許先開代理；不可用時標記未驗證、阻塞或總監風險關閉但非完整 |

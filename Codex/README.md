...
| **編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造先建立 Programming Team Board；每站必須標示是否適用與執行模式；Codex native subagents 只在站點允許、總監明確要求或專案代理已配置時做唯讀 evidence branch |
...
| **Programming team governance coverage** | Doctor 檢查編程團隊治理共用技能、團隊站點板、三平台 workflow 接入與部署後 shared skill / shared reference hash 是否一致 |

...
.EXAMPLE
    ...
    $repo = 'AI_Rules'
    $ref = 'main'
    $platformPath = 'Antigravity/install.ps1'
    $expectedInstallSha256 = ''
    $installerArgs = @()
    if ($ref -notmatch '^[A-Za-z0-9._-]+$' -or $ref.Contains('..')) { throw "Unsafe Antigravity install ref: $ref" }
    $u = "https://raw.githubusercontent.com/$owner/$repo/$ref/$platformPath"
    $uri = [Uri]$u
    if ($uri.Scheme -ne 'https' -or $uri.Host -ne 'raw.githubusercontent.com' -or $uri.AbsolutePath -ne "/$owner/$repo/$ref/$platformPath") { throw "Unexpected Antigravity install source: $u" }
    $f = Join-Path $env:TEMP 'ag_install.ps1'
    $receipt = Join-Path $env:TEMP 'ag_install.receipt.json'
    try {
        ...
        if ($expectedInstallSha256 -and $actualInstallSha256 -ne $expectedInstallSha256.ToLowerInvariant()) { throw "Antigravity install script SHA256 mismatch." }
...
$receiptPath = if ([string]::IsNullOrWhiteSpace($DownloadReceipt)) {
    Join-Path $env:TEMP "antigravity_receipt_$([Guid]::NewGuid().ToString('N')).json"
} else {
...
$tempName = "antigravity_$([Guid]::NewGuid().ToString('N'))"
...
try {
    ...
    if ($Mode -eq "Menu") {
        ...
        & $deployScript -Target $targetPath
    } elseif ($RemoveOrphans) {
        & $deployScript -Platform Antigravity -Mode $Mode -Target $targetPath -RemoveOrphans
    } else {
        & $deployScript -Platform Antigravity -Mode $Mode -Target $targetPath
    }

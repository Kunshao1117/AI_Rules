...
3. **角色權限模糊** — 子代理人隨意改檔案，或 AI 形式上列站點卻仍全部主線直做 → 編程意圖自動進入隊長制團隊模式，先產出草案包，GO 後轉正式派工板並逐波次派工；證據型站點預設使用唯讀 evidence branch，實作隊員只能在受治理隔離區產出變更交付件，或在無隔離時產出文字變更交付件，審查者不能實作同一交付物；隊長任務只限總監溝通、授權解析、建板、派工、監督、有限驗讀、受保護採納或合入已回收且合格的交付件、狀態裁決與回報
4. **知識碎片化** — 技能散落各處，Token 暴增 → 61 套按需載入的操作型技能，不用時零開銷
...
| **Team-Native Core 團隊原生核心** | 開發、修改、修復、測試、除錯、健檢、提交、交接、技能鍛造，以及會影響後續建構的 00 證據型對話、探索、架構、廣泛讀檔與外部研究，都會依語意自動進入 Team-Native Core。這不是文字建議，而是前置執行狀態機：下一個合法狀態必須是隊長任務板、適用站點、隊員派工包與通道狀態。Antigravity / Gemini 唯讀工作使用正式唯讀板，寫入工作必須有 GO 或介面確認支撐的正式寫入板；每站都要記錄隊員技能引用、深讀範圍、隊長驗讀範圍、啟動期限、待命原因或阻塞原因。00 直答只保留給不需外部證據的純聊天；涉及檔案、截圖、記憶、規則、代理行為、工具輸出或後續治理影響時，必須升級為正式唯讀團隊站點。站點以 adapter 或 conditional 路由為主，可轉成 Gemini CLI、`@` 指派、browser-capable agent、IDE workflow、plugin adapter、瀏覽器或命令列證據；若通道不可用，必須先回報 unavailable、blocked、unverified、not-authorized 或 standby，不得默默降級成 routine direct。正式團隊完成必須回收 implementation change delivery、memory delivery、review、validation 四類交付件與 Team-Native trace；隊長最後只保護性採納或合入已回收且合格的變更交付件與證據，不能先自行讀完、實作、審查或驗證再事後補團隊軌跡 |
...
| 03-2 | 建構執行 | Stage 2：正式派工板 → 第 1 波變更交付件 → 回收證據 → 達到下一波啟動條件後再派審查/驗證交付件 → 隊長保護性採納或合入合格交付件 → 新建歸卡 → 記憶更新 → 真實執行驗證 | Captain/SRE |
| 05 | 濃縮 | 專案濃縮初始化（掃描 → 萃取 → 隊長任務板 → 審閱 → 寫入） | Writer/SRE |
| 04-1 | 修復計畫 | Bug 診斷 → 產出修復計畫（唯讀，等待 GO） | Reader |
| 04-2 | 修復執行 | 正式派工板 → 第 1 波修復變更交付件 → 回收證據 → 達到下一波啟動條件後再派審查/回歸驗證交付件 → 隊長保護性採納或合入合格交付件 → 記憶更新 → 真實失敗路徑回歸測試 | Captain/SRE |
...
| 09-2 | 授權備份 | 正式派工板 → 審查/收尾證據交付件分波回收 → 隊長文件更新 + 明確清單 Git 提交 + 遠端推播 | Captain/SRE |
...
技能是**按需載入的知識手冊**。IDE 在對話開始時僅注入技能名稱與描述，完整內容在需要時才讀取，實現漸進式揭露。新版共用技能總數為 61 套，其中新增專家角色母技能、十個專家子技能，並保留既有團隊交付技能；專家分類、變更交付、記憶交付、驗證、審查與完成閘門由隊員分工交付。
...
| | `audit-engine` | 深層健檢語義引擎（深度模式、專案型態、盤點分母、證據交付件、覆蓋率、燈號規則、安全/API/資料流與相容性） | 英文指令（中文註解） | — |

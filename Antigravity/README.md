...
4. **知識碎片化** — 技能散落各處，Token 暴增 → 40 套按需載入的操作型技能，不用時零開銷
...
Antigravity 版正式建構、修復、測試與健檢都套用「可驗必驗」原則。只要 AI 能啟動應用、操作介面、呼叫服務、執行命令、查詢資料、讀取日誌、截圖或觀察副作用，就必須實際驗證；mock、fixture、假資料、靜態截圖或局部單元測試只能作為局部證據。缺少真實執行證據時，結果必須是不通過或阻塞，不能包裝成完成。允許阻塞的條件限於缺少外部授權、憑證、實體設備、不可安全執行的破壞性動作、第三方服務不可用、MFA/CAPTCHA、法規或安全限制。

Antigravity 健檢入口採全光譜證據式架構：先偵測網站、後端、命令列、桌面、外掛、函式庫、基礎設施、資料管線、AI 功能、文件治理庫或混合專案，再依平台能力掛載基礎、治理、邏輯、真實操作、效能、發布與相容性檢查。Antigravity 版優先使用代理管理、瀏覽器子代理、截圖、錄影與視覺產物採證；中繼證據只允許寫入健檢日誌，不授權修改原始碼、記憶卡或外部狀態。
...
| 08 | 專案健檢 | 專案型態偵測、平台能力快照、動態掛載模組與證據式健康報告 | Reader/Logs |
...
| | `audit-engine` | 全光譜健檢語義引擎（專案型態、證據包、燈號規則、安全/API/資料流與相容性） | 英文指令（中文註解） | — |

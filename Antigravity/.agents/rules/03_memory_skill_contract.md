...
│   └── YES → Record override/risk-closure request only; keep the exit hold and memory/source attribution gates active. [SUDO] does not authorize completion claims.
...
- **Memory Directory**: Source-code memory is stored in `.agents/memory/` as active memory cards. The target canonical main file is `MEMORY.md`; existing `SKILL.md` files under `.agents/memory/` are legacy compatibility sources until governed migration and cartridge-system support are confirmed. Memory cards are not executable skills.
- **Project Context Directory**: Long-lived preferences, design DNA, product defaults, communication preferences, and acceptance preferences live in `.agents/context/**/CONTEXT.md`. These cards are protected on upgrade, are not executable skills, and do not participate in source memory staleness.
- **Readable AND Writable**: Unlike operational skills, memory cards are read-write. After modifying source files tracked by a memory card, update the relevant sections of its active memory main file.
...
- **Context Write Gate**: Project context can be proposed during normal workflows, but persistent writes require `GO CONTEXT`; design DNA may use `GO DNA` as an alias.
...
- **Timestamp Standard (時間戳標準)**: ALL timestamps in memory card frontmatter MUST use **ISO 8601 with Taiwan timezone**: `YYYY-MM-DDTHH:mm:ss+08:00`. Using UTC (`Z` suffix) is FORBIDDEN.
...
- **Knowledge Layer Architecture（知識層架構）**: The framework separates executable skills from non-executable project context:
...
- **Sanitization (淨化義務)**: Before writing to ANY memory card, you MUST scrub all PII (Personally Identifiable Information), absolute system paths containing user names (e.g., `C:\Users\JohnDoe\`), and secret tokens/API keys. Memory cards must contain only architectural knowledge and relative paths.
- **Purging (汰除機制)**: If a module is entirely deleted from the codebase, you MUST physically delete its corresponding memory card from `.agents/memory/`. Do NOT leave abandoned memory cards, as they permanently pollute the AI's source context. Project context should be marked `deprecated` unless the Director explicitly approves deletion.

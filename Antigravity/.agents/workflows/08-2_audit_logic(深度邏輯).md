...
description: "健檢第二階段、依盤點清單做深度邏輯審查、安全架構、API/端點/資料流比對、真實功能驗證、視覺/瀏覽器採證、效能可靠性、測試覆蓋缺口與死碼偵測（Use when: audit phase 2 logic review）。不要用於完整健檢入口；改用 `08_audit`（DO NOT use when: full audit entry is needed）。"
required_skills:
  - delegation-strategy
  - code-audit
  - audit-engine

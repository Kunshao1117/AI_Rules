---
description: "全光譜專案健檢、深層 audit、證據式健檢、專案型態偵測、功能/端點/命令盤點、相容性檢查、治理巡檢、基礎盤點、深度邏輯審查、視覺/瀏覽器採證、效能與載入速度、健康報告（Use when: full-spectrum audit）。不要用於單一測試或單一 bug 修復（DO NOT use when: single test or single bug fix）。"
...
6. When a concrete phase checklist is needed, read the deployed stage procedure reference (`.agents/shared/workflow-stage-procedures.md`) and framework source reference (`Shared/workflow-stage-procedures.md`), then use section `08 Audit`. Do not copy that procedure back into this entry.
...
- Route summary: Run inventory before logic review, logic review before final report, and route repairable findings to the owning workflow.
- Define audit depth, project type, surface denominator, evidence sources, and unavailable areas.
- Run 08-1 inventory before 08-2 logic review, and 08-2 before 08-3 report.
- Do not repair during audit; findings route to build, fix, test, skill-forge, commit prep, or handoff.
- Report red/yellow/green only with evidence status, blockers, and unverified scope.

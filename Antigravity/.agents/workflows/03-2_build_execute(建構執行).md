---
description: "建構執行、套用 `/03_build` design-to-build contract、已完成範圍綁定授權解析後的建構寫入、記憶歸屬與驗證測試（Use when: build execution after resolved scope）。不要用於缺少建構合約或意圖訊號尚未解析到可見範圍的情況（DO NOT use when: contract or resolved intent is missing）。"

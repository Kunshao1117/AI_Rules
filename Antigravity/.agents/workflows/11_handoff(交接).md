---
description: "交接、handoff、彙整目前對話成果、掃描記憶卡並產出下一個 AI 可接手的提示詞（Use when: handoff and continuation prompt）。不要用於仍在實作或需要提交（DO NOT use when: implementation is active or commit work is needed）。"

...
推送 tag `v0.1.16` 後，GitHub Actions 會自動建立 GitHub Release，打包 `ai-rules-manager-0.1.16.vsix`，附加到該 release 的 Assets，並從 `CHANGELOG.md` 的對應 `AI Rules Manager v<version>` 段落產生 Release 簡介。Release workflow 使用 Node 24 與支援 Node 24 runtime 的官方 actions，避免 GitHub Actions Node 20 淘汰造成發布風險。若 tag 與 `Extensions/vscode-ai-rules-manager/package.json` 的版本不一致，workflow 會直接失敗，避免放錯插件包。需要補跑時，也可以在 GitHub Actions 頁面手動執行 workflow 並輸入 tag。
...
| **介面適配證據** | UI 驗收依介面類型選擇證據：網頁看 viewport，桌面 GUI 看視窗縮放與高 DPI，外掛面板看容器與主題，終端工具看輸出與失敗碼 |
| **真實執行證據** | 跨專案驗收預設可驗必驗；能啟動、操作、呼叫、查詢、截圖、讀日誌或觀察副作用時，AI 必須實測，假資料與靜態截圖只能作為局部證據 |
...
記憶卡是來源記憶，不是可執行技能。目標標準主檔為 `MEMORY.md`；既有專案仍可能在相容期保留 `.agents/memory/**/SKILL.md`，正式更名必須等 cartridge-system 支援確認後，透過管理器的記憶主檔遷移入口乾跑、檢查衝突，再另行授權套用。本專案目前尚未執行實體記憶卡主檔更名。

記憶主檔遷移工具的預設模式只做盤點，不會更名或改寫記憶卡：

```powershell
.\Scripts\AI-RulesManager.ps1 -Action MemoryMigration -Target .
```

正式套用更名需另一次明確授權並加上 `-Apply`；在外部記憶引擎支援新主檔前，不應對正式專案執行。
...
下游專案不會取得完整 `Scripts/` 或 `Shared/` 來源樹。部署與專案同步只會把 AI 執行規則所需的唯讀治理參考複製到 `.agents/shared/`，包含平台能力矩陣、工作流證據矩陣、技能治理規格、子代理政策與 MCP opt-in 片段。會修改狀態的工具操作，例如記憶主檔遷移、同步套用或孤兒清理，必須透過 AI Rules Manager 來源倉庫或 VS Code 外掛執行。

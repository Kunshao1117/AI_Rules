...
安裝後，VS Code 左側 Activity Bar 會出現 **AI Rules**。側邊欄提供 6 個按鈕：
...
| **健康檢查** | 呼叫 `Deploy.ps1 -Action Audit` 的治理巡檢能力 |
| **同步全域規則** | 先預覽差異，確認後才寫入 `~/.codex`、`~/.claude`、`~/.gemini` |
...
|------|---------|---------|--------|--------|-----------|---------| 
| **Antigravity** | Gemini（IDE 插件 + CLI） | v8.0.0 | 9 | 20 | 36 | [Antigravity/README.md](Antigravity/README.md) |
| **Claude Edition** | Claude Code（VS Code 插件） | v1.2.0 | 7 | 14 | 36 | [Claude/README.md](Claude/README.md) |
| **Codex Edition** | OpenAI Codex（agentskills.io 標準）| v0.1.0 | 1 | 17 | 36 | [Codex/README.md](Codex/README.md) |
...
AI_Rules 的計畫與報告預設使用「功能/目的、相關檔案、白話說明、寫入/風險」表格，讓非工程背景使用者也能核對變更。技術細節仍可保留，但必須放在表格後的「補充技術細節」段落。
...
| **VS Code 管理器** | 將常用治理動作包成側邊欄按鈕；寫入、更新、清理都需確認 |

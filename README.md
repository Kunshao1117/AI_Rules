...
| **Team-Native Core 團隊原生核心** | 開發、修改、修復、測試、除錯、健檢、提交、交接、技能鍛造，以及會影響後續建構的探索、架構、廣泛讀檔與外部研究，都會依語意自動進入團隊原生核心。這不是文字建議，而是前置執行狀態機：下一個合法狀態必須是隊長任務板、適用站點、隊員派工包與通道狀態。草案板只用於需求、範圍與站點建議；唯讀工作轉正式唯讀板，寫入工作必須有 GO 支撐的正式寫入板。每個正式站點都要有隊員技能引用、深讀範圍、隊長驗讀範圍、禁止動作、輸出格式、啟動期限、待命原因或阻塞原因；隊員可待機，但待機不是證據。隊長只負責協調、派工、監督、整合已回收交付件與回報，不得先大量讀檔、實作、審查、驗證或記憶歸因，再事後補團隊軌跡。若平台無法開啟隊員通道，必須先回報不可用、未授權、阻塞、未驗證或待命與最小解阻條件，不得默默降級為例行主線直做。正式團隊完成必須回收 implementation change delivery、memory/docs delivery、review、validation 四類交付件與 Team-Native trace；缺任一合格交付件只能標示阻塞、未驗證或總監風險關閉但非完整，不可稱完整完成 |

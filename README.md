...
| **全光譜證據式健檢** | 健檢先判定專案型態與平台能力，再動態掛載基礎、治理、邏輯、真實操作、效能、發布與相容性檢查；缺證據時只能標記未驗證或阻塞 |
...
| 全光譜健檢架構 | 共用審計引擎保存專案型態矩陣、證據包格式與報告燈號規則 | 三平台保留既有入口，但先做型態偵測與能力快照，再依專案表面掛載檢查；中繼證據只寫入健檢日誌，不改原始碼、記憶卡或外部狀態 |
| 工作流外部接地矩陣 | `Shared/workflow-capability-evidence-matrix.md` | 08 以外工作流不再各自散落最佳實務；每個入口都有任務型態、外部依據、最低證據、平台差異與下一流程路由 |
...
| **證據式健檢日誌** | 健檢工作流可在 `.agents/logs/audit/<timestamp>/` 寫入專案設定檔、證據包與總結；此權限不等於可修改原始碼、記憶卡、部署或外部服務 |

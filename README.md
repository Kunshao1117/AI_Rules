...
3. **角色權限模糊** — 子代理人隨意修改檔案，或 AI 形式上列站點卻仍全部主線直做。→ 編程意圖自動進入隊長制團隊模式，先產出草案包，GO 後轉正式派工板並逐波次派工；證據型站點預設交給唯讀證據分支，實作隊員只能在受治理隔離區產出變更交付件，或在無隔離時產出文字變更交付件，審查者不能實作同一交付物；隊長只協調、派工、監督、保護性合入已回收且合格交付件與回報，不能把 GO 解讀成整批未限範圍寫入

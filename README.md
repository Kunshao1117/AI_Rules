...
3. **角色權限模糊** — 子代理人隨意修改檔案，或 AI 形式上列站點卻仍全部主線直做。→ 編程意圖自動進入隊長制團隊模式，先產出草案包，GO 後轉正式派工板並逐波次派工；證據型站點預設交給唯讀證據分支，實作隊員只能在受治理隔離區產出補丁，或在無隔離時產出文字補丁任務包，審查者不能實作同一交付物；所有主工作區寫入與交付仍由隊長整合
4. **知識碎片化** — 技能散落各處，Token 消耗暴增。→ 50 套按需載入的操作型技能，不用時零開銷
...
| **隊長制編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入隊長制，不要求總監手動指定工作流；總監只對隊長說話。新版把隊長主線處理拆成團隊協作：隊長只負責調度、整合、裁決與總監溝通；隊員分別負責實作補丁、記憶交付、驗證、審查、完成閘門等單一任務。隊長收到需求後先產出草案包；草案板只用於需求、範圍與站點建議，不得啟動隊員，也不得把草案證據算作正式驗收。收到 GO 後才轉成正式派工板，並依波次啟動隊員；每一波必須以上一波回收的補丁、審查或驗證證據作為輸入，且滿足下一波啟動條件。正式團隊完成必須回收 implementation patch、memory delivery、review、validation 四類封包；隊長最後只整合已回收且可追溯的補丁與證據，並保留主線寫入、審查狀態、記憶、提交、發布與最終驗收 |
| **自治分級治理** | AI 先依使用者可見影響、資料風險、公開介面、跨平台與回復成本判定治理深度；無法證明輕量時預設升級 |
| **變更意圖分類** | 正式建構、修復、測試與健檢必須區分緊急修補、根因修復、局部修整與結構重構；同一症狀或區域重複補丁時，必須升級為根因修復或結構重構 |
...
| **最小權限治理** | 角色分層（讀取者 / 工作者 / 寫入者），子代理啟用政策由 `Shared/policies/` 同源轉譯；隊員必須在隊長任務板後啟動，證據型隊員只能唯讀採證，實作型隊員只能在受治理隔離區產出補丁包，不能直接改主工作區、提交、推送、發布或改記憶；缺少分支證據或任務包時必須標示未驗證、阻塞或隊長代工風險接受 |
...
| 深層證據式健檢架構 | 共用審計引擎保存深度矩陣、專案型態矩陣、盤點契約、型態食譜、證據包格式與報告燈號規則 | 三平台保留既有入口，但先做深度選擇、型態偵測、能力快照與盤點分母，再依專案表面掛載檢查；中繼證據只寫入健檢日誌，不改原始碼、記憶卡或外部狀態 |
...
| 平台能力矩陣 | 來源：`Shared/platform-capability-matrix.md`；部署：`.agents/shared/platform-capability-matrix.md` | 用 `native` / `adapter` / `manual` 標記三平台能力，不假設三者完全對等 |
...
| 團隊任務包模板 | 來源：`Shared/skills/team-task-package/SKILL.md`；部署：`.agents/skills/team-task-package/SKILL.md` | 集中保存輕量任務板、完整任務板、實驗任務板、專員任務包、證據包、隔離補丁包、文字補丁包、直接處理例外與收尾檢查表；工作流只引用模板，不再內嵌長段規則 |
| 團隊協作子技能 | 來源：`Shared/skills/team-role-boundaries/`、`Shared/skills/implementation-patch-delivery/`、`Shared/skills/memory-coupled-delivery/`、`Shared/skills/team-validation-packet/`、`Shared/skills/team-review-packet/`、`Shared/skills/team-completion-gate/`；部署：`.agents/skills/` | 將角色邊界、實作補丁、記憶交付、驗證封包、審查封包與完成閘門拆成單一責任隊員技能，讓正式完成不再由隊長主線自行包辦 |
| 子代理治理模型 | 來源：`Shared/policies/subagent-invocation.md`；部署：`.agents/shared/policies/subagent-invocation.md` | Shared 只定義 Delegation Gate、evidence branch、isolated patch、角色互斥、假團隊防線與主線直做例外語義；Codex、Claude、Antigravity 各自由 adapter 轉成平台工具 |
...
graph TB
    ...
    subgraph "目標專案（三版本可共存）"
        ...
        CXE[".codex/ + .agents/skills/<br/>治理規則 + 技能（67 套）"]
...
| **子代理人** | 隊長任務板建立後 → Gemini CLI / `@` 指派 / browser-capable agent / Antigravity plugin adapter / 受治理隔離補丁；不可用時標記未驗證、阻塞或隊長代工風險接受 | 隊長任務板建立後 → description-driven subagent / `@agent` / governed `Agent(...)` / 受治理隔離補丁；不可用時標記未驗證、阻塞或隊長代工風險接受 | 隊長任務板建立後 → Codex native subagents 或 `.codex/agents/*.toml` / 受治理隔離補丁；Director 要求只會強制建板派工，不允許先開代理；不可用時標記未驗證、阻塞或隊長代工風險接受 |
...
| **技能總數** | 50 套 | 50 套 | **67 套**（50 共用 + 17 工作流） |
...
│   └── skills/                        ← 50 套操作型技能唯一真實來源，部署時注入各平台
│
...
| **隊長制編程治理覆蓋審計** | 治理巡檢 Doctor 會掃描隊長制編程治理技能、團隊任務包模板、子代理政策、委派策略、能力矩陣、工作流證據矩陣、00/01 觸發轉向、05 濃縮寫入入口與三平台編程入口，確認任務類型閘門、派工前置閘門、隊長最小執行權規則、隊長任務板、角色邊界、隔離補丁、文字補丁、證據負責人、主線直做例外、全主線假團隊防線與部署副本一致；Doctor 也會檢查正式派工生命週期、逐波次派工、上一波輸入、下一波啟動條件、正式證據資格，並攔截草案板啟動隊員、草案證據冒充正式驗收、正式板後一次開全部隊員 |
| **Experiment 例外** | `03-1_experiment` 是刻意設計的沙盒例外，允許快速寫檔並跳過正式品質、測試與記憶收尾，但仍必須先列最小隊長任務板、沙盒邊界、允許改動範圍、丟棄條件、升級條件、證據負責人、角色邊界、主線直做例外與隔離補丁條件，且不可標為 automation-safe |
...
| **證據式健檢日誌** | 健檢工作流可在 `.agents/logs/audit/<timestamp>/` 寫入專案設定檔、盤點清單、證據包與總結；此權限不等於可修改原始碼、記憶卡、部署或外部服務 |
...
50 套操作型技能存放於 `Shared/skills/`（唯一真實來源），部署時由 `Scripts/modules/Skills-Sync.psm1` 注入各平台：

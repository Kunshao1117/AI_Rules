...
3. **角色權限模糊** — 子代理人隨意修改檔案，或 AI 形式上列站點卻仍全部主線直做。→ 編程意圖自動進入隊長制團隊模式，先產出草案包，GO 後轉正式派工板並逐波次派工；證據型站點預設交給唯讀證據分支，實作隊員只能在受治理隔離區產出變更交付件，或在無隔離時產出文字變更交付件，審查者不能實作同一交付物；隊長只協調、派工、監督、整合已回收交付件與回報
...
這些介面按鈕的確認視窗可作為授權證據，但授權範圍只限當下視窗明示的動作、目標專案、差異清單、命令或檔案集合。按下同意不是無範圍寫入，也不會自動授權記憶卡、提交、推送、發布、部署、安裝、憑證或外部狀態變更。
...
| **Team-Native Core 團隊原生核心** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入團隊原生核心，不要求總監手動指定工作流；總監只對隊長說話。隊長制不是單人主線加可選子代理，而是交付件驅動的站點優先狀態機：隊長只負責協調、派工、監督、整合已回收交付件與回報；隊員分別負責實作變更交付、記憶文件交付、驗證交付、審查交付、完成閘門等單一任務。隊長收到需求後先產出草案包；草案板只用於需求、範圍與站點建議，不得啟動隊員，也不得把草案證據算作正式驗收。收到 GO 後才轉成正式派工板，並依波次啟動隊員；每一波必須以上一波回收的交付件或證據作為輸入，且滿足下一波啟動條件，不得一次全開。每個適用站點都要記錄平台能力路由：native、adapter、conditional 或 unavailable；conditional 未證明可用時只能標示未驗證、阻塞或總監風險關閉但非完整，不得變成例行主線直做。正式團隊完成必須回收 implementation change delivery、memory/docs delivery、review、validation 四類交付件與 Team-Native trace；缺任一合格交付件只能標示阻塞、未驗證或總監風險關閉但非完整，不可稱完整完成。隊長不得產出主要實作、審查、驗證或記憶歸因，也不得用自審完成審查 |
| **範圍式授權解析** | GO、介面按鈕、平台權限提示、模式切換與工作流指令都必須解析成明確動作、階段、站點、命令、工具或檔案集合；工作流只做路由，按鈕同意只作為該視窗明示範圍的授權證據，不是無範圍寫入或受保護狀態操作 |

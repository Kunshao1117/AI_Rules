...
3. **角色權限模糊** — 子代理人隨意修改檔案，或 AI 形式上列站點卻仍全部主線直做。→ 使用者要求 governance、workflow、fix、build、debug、test、audit、skill、memory/docs、commit、handoff、source、public-contract 等受治理工作時，即視為使用者要求啟動隊長制團隊模式；不需固定口令。
   Team mode 啟動只建立治理與派工狀態，不授權寫入；寫入仍需 visible plan、file scope、authorization phase、exact allowlist、dirty diff read 與 protected gate。正式執行先產出草案包，範圍綁定後轉正式派工板並逐波次派工；主工作區 implementation 由具名 station-owned `change-delivery` 站點寫入，`change-application` 只作 returned artifact、明確 integration task 或 assigned sync 的 fallback；隊長只協調、派工、監督、接收站點交付、更新任務板、彙整狀態與回報，不能把 GO 解讀成整批未限範圍寫入
...
推送 tag `v0.2.0` 後，GitHub Actions 會自動建立 GitHub Release，打包 `ai-rules-manager-0.2.0.vsix`，附加到該 release 的 Assets，並從 `CHANGELOG.md` 的對應 `AI Rules Manager v<version>` 段落產生 Release 簡介。Release workflow 使用 Node 24 與支援 Node 24 runtime 的官方 actions，避免 GitHub Actions Node 20 淘汰造成發布風險。若 tag 與 `Extensions/vscode-ai-rules-manager/package.json` 的版本不一致，workflow 會直接失敗，避免放錯插件包。需要補跑時，也可以在 GitHub Actions 頁面手動執行 workflow 並輸入 tag；若同名 VSIX asset 已存在，workflow 會拒絕或失敗，需改用新版本/tag，或先人工刪除舊 asset 後再重跑。
...
三個版本的**操作型技能均源自 `Shared/skills/`**（唯一真實來源，62 套），記憶系統依賴 [cartridge-system](https://github.com/Kunshao1117/cartridge_system)，並透過 [Multi-MCP Gateway](https://github.com/Kunshao1117/Multi-MCP) 統一探索與呼叫下游 MCP 工具（外部依賴）。專案脈絡層使用 `.agents/context/` 保存設計 DNA、產品偏好、技術偏好、溝通偏好與驗收偏好，不走記憶 stale。
...
| **總監輸出與接地雙閘門** | 平台核心只保留「總監可讀繁中語義先行」與「高變動或外部事實需接地查證」兩個最小硬句；細節引用 `Shared/policies/language-governance.md` 與 `Shared/policies/grounding-governance.md`。治理文件採 source/deployed sync：先改框架 source，再同步 runtime copy 並驗證雜湊一致 |
| **最小權限治理** | 角色分層（讀取者 / 工作者 / 寫入者），子代理啟用政策由 `Shared/policies/` 同源轉譯；未啟動 Team mode 時依一般生命週期、範圍式授權與 protected gates 處理。Team mode 啟動後，隊員必須在隊長任務板後啟動，證據型隊員只能唯讀採證；主工作區實作的 primary 路徑是具名 station-owned `change-delivery`，需 formal-write、精確 allowlist、dirty diff read 與 no protected actions；isolated/text-only 只作無法直接主工作區委派時的 fallback，`change-application` 只處理 returned isolated/text artifact、explicit integration task 或 assigned generated/deployed sync；缺少分支證據、任務板或合格交付件時必須標示未驗證、阻塞或總監風險關閉但非完整 |
...
graph TB
    ...
    subgraph "目標專案（三版本可共存）"
        ...
        CXE[".codex/ + .agents/skills/<br/>治理規則 + 技能（79 套）"]
...
| **隊長制編程治理覆蓋審計** | 治理巡檢 Doctor 會掃描隊長制編程治理技能、團隊任務板模板、子代理政策、委派策略、能力矩陣、工作流證據矩陣、受治理請求觸發語意、05 濃縮寫入入口與三平台編程入口，確認任務類型閘門、派工前置閘門、隊長最小執行權規則、隊長任務板、角色邊界、station-owned main-worktree change delivery、fallback change-application、證據負責人、主線直做例外、全主線假團隊防線與部署副本一致；Doctor 也會檢查正式派工生命週期、逐波次派工、上一波輸入、下一波啟動條件、正式證據資格，並攔截草案板啟動隊員、草案證據冒充正式驗收、正式板後一次開全部隊員 |
| **Team-Native Core 審計** | 治理巡檢 Doctor 會掃描團隊原生核心政策、任務軌跡契約、平台能力路由、工作流證據矩陣與部署後共用參考；必要時可要求任務軌跡，缺少 Team-Native trace 會標示未驗證或阻塞 |
| **Experiment 例外** | `03-1_experiment` 是受治理的沙盒 workflow；使用者要求 03-1、experiment、sandbox prototype、spike 或髒碼原型時，Team mode 由該請求觸發，主線擔任隊長並使用 reduced/minimal experiment station/board。它允許快速 sandbox writes 與跳過正式品質、測試與記憶收尾，但必須先記錄 sandbox scope、允許改動範圍、丟棄條件、升級條件、allowed shortcuts、experiment-only 處置與 scope-bound authorization。Sandbox writes、discard 與 promotion 不等於 production completion；promotion to 03/build 或 production source/governance/public-contract write 需要新的 production scope-bound authorization、`formal-write`、station-owned `change-delivery`、validation、review、memory/docs，且不可標為 automation-safe |

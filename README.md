...
> **讓 AI 編碼助手不再失憶、不再無紀律** — 零接觸自動部署的 AI 代理人治理框架，為 Gemini 與 Claude Code 提供統一的工作流程、持久記憶系統與標準作業規範。
...
2. **無紀律執行** — 寫碼前不規劃、寫完不測試、改完不更新文件。→ 19 道生命週期工作流強制「規劃→閘門→執行→歸檔」四拍子
...
6. **雙 AI 記憶分歧** — Gemini 和 Claude Code 各自記各自的。→ `.agents/memory/` 統一記憶庫，兩個 AI 共用同一份記憶

...
3. **角色權限模糊** — 子代理人隨意修改檔案，或 AI 形式上列站點卻仍全部主線直做。
   ...
   `change-application` 只作 returned artifact、明確 integration task 或 assigned sync 的 fallback。
   隊長只協調、派工、監督、接收站點交付、更新任務板、彙整狀態與回報，不能把 GO 解讀成整批未限範圍寫入。
4. **知識碎片化** — 技能散落各處，Token 消耗暴增。→ 62 套按需載入的操作型技能，不用時零開銷
...
選擇你的 AI 編碼助手，在專案目錄的終端機中複製對應指令區塊即可安裝：

> 相容性：以下 PowerShell 指令支援 Windows PowerShell 5.1+ 與 PowerShell 7。
> 指令會將 GitHub 下載內容以 UTF-8 解碼，並以 UTF-8 BOM 寫入暫存腳本，避免舊版中文 Windows PowerShell 將腳本誤判為 ANSI/Big5。
...
> 三個版本可以安裝到**同一個專案**中共存。
>
> Gemini 使用 `.agents/`，Claude Code 使用 `.claude/`，Codex 使用 `.codex/`，互不衝突。
> 三者透過 `.agents/memory/` 共用原始碼記憶，並透過 `.agents/context/` 共用專案脈絡。
...
AI_Rules 也提供本機 VS Code 延伸模組管理器，適合不想記 PowerShell 指令的日常維護場景。
第一版以 `.vsix` 本機安裝與 GitHub Release asset 分享為主；未來可再上架 Marketplace。
手動安裝 VSIX 不等於 Marketplace 原生自動更新，因此延伸模組啟動時會查 GitHub Release；只有發現新版安裝檔時才通知，沒有新版或暫時無法連線時只寫入 Output Channel，不打擾操作者。
...
這些介面按鈕的確認視窗可作為授權證據，但授權範圍只限當下視窗明示的動作、目標專案、差異清單、命令或檔案集合。
總監可以用「繼續」「先處理這個」「照這樣做」等日常語句授權，但模型必須先把語句綁定到目前可見的計畫、站點、檔案、命令、差異或阻塞點；無法綁定時只算路由意圖或部分證據。
按下同意不是無範圍寫入，也不會自動授權記憶卡、提交、推送、發布、部署、安裝、憑證或外部狀態變更。
...
在 Antigravity、VS Code 或相容 IDE 中，如果目前 workspace 不是 AI_Rules repo，延伸模組會使用該 IDE `globalStorage` 內的 AI_Rules 管理快取作為來源。
這份快取被視為遠端版本庫鏡像：每次執行管理動作前會自動對齊 `aiRules.repoUrl` 的 `main` 分支；若已明確設定 `aiRules.repoRoot`，則該本機來源只檢查狀態，不會被自動重設。
使用者層規則檢查以文字內容為準；同一份規則只因 Git/Windows 將換行存成 LF 或 CRLF 時，不會被視為需要同步的漂移。
同步目前專案規則時，05 濃縮工作流寫入的 `PROJECT IDENTITY` 區段會被保留，只更新框架管理內容。

`aiRules.repoRoot`、`aiRules.repoUrl` 與 `aiRules.powerShellPath` 只能放在 VS Code 使用者設定，不能放在專案工作區設定。
若陌生專案嘗試用工作區設定改寫 AI_Rules 來源或 PowerShell 執行檔，延伸模組會停止，避免用不可信來源同步規則。

### GitHub Release 自動建立與附加 VSIX

推送 tag `v0.2.0` 後，GitHub Actions 會自動建立 GitHub Release。
流程會打包 `ai-rules-manager-0.2.0.vsix`，附加到該 release 的 Assets。
Release 簡介會從 `CHANGELOG.md` 的對應 `AI Rules Manager v<version>` 段落產生。
Release workflow 使用 Node 24 與支援 Node 24 runtime 的官方 actions，避免 GitHub Actions Node 20 淘汰造成發布風險。
若 tag 與 `Extensions/vscode-ai-rules-manager/package.json` 的版本不一致，workflow 會直接失敗，避免放錯插件包。
需要補跑時，也可以在 GitHub Actions 頁面手動執行 workflow 並輸入 tag；若同名 VSIX asset 已存在，workflow 會拒絕或失敗，需改用新版本/tag，或先人工刪除舊 asset 後再重跑。

---

## 📦 框架版本總覽

| 版本 | 目標平台 | 當前版號 | 規則數 | 工作流 | 共用技能 | 詳細文件 |
|------|---------|---------|--------|--------|-----------|---------| 
| **Antigravity** | Gemini（IDE 插件 + CLI） | v8.0.3 | 9 | 20 | 62 | [Antigravity/README.md](Antigravity/README.md) |
| **Claude Edition** | Claude Code（VS Code 插件） | v1.2.3 | 7 | 17 | 62 | [Claude/README.md](Claude/README.md) |
| **Codex Edition** | OpenAI Codex（agentskills.io 標準）| v0.1.3 | 1 | 17 | 62 | [Codex/README.md](Codex/README.md) |

三個版本的**操作型技能均源自 `Shared/skills/`**（唯一真實來源，62 套）。
記憶系統依賴 [cartridge-system](https://github.com/Kunshao1117/cartridge_system)。
記憶系統也透過 [Multi-MCP Gateway](https://github.com/Kunshao1117/Multi-MCP) 統一探索與呼叫下游 MCP 工具（外部依賴）。
專案脈絡層使用 `.agents/context/` 保存設計 DNA、產品偏好、技術偏好、溝通偏好與驗收偏好，不走記憶 stale。
...
| **總監輸出與接地雙閘門** | 繁中語義先行；高變動或外部事實需以本地檔案、工具輸出或主要來源接地 |
| **最小權限治理** | 角色分層、子代理唯讀採證、主工作區寫入走具名 `change-delivery` 或 fallback gate |
| **三位一體治理** | 靜默異常中斷、`[SUDO]` 風險紀錄、沙盒路徑互相分離，不能跳過授權與完成證據 |
| **閘門即防護** | 偵測到異常時才輸出中斷訊息，正常通過時零輸出，不干擾開發流程 |
| **雙受眾設計** | AI 看英文指令層、總監看中文介面層，兩者共讀橋接層 |

Team-Native Core 補充：
- 觸發來源包含 governance、workflow、fix、build、debug、test、audit、skill、memory/docs、commit、handoff、source 與 public-contract。
- 純對話、小型穩定問答與無 source/governance/evidence 影響的工作可維持 direct。
- Team mode 啟動不等於寫入授權；下一合法狀態需有任務板、站點、派工包與通道狀態。
- 寫入仍需 visible plan、file scope、authorization phase、exact allowlist、dirty diff read 與 protected gate。
- 主工作區 implementation 由 station-owned `change-delivery` 寫入。
- `change-application` 只作 returned artifact、明確 integration task 或 assigned generated/deployed sync 的 fallback。
- 隊長只做溝通、授權解析、建板、派工、監督、交付接收、狀態彙整、阻塞處理與回報。
- 隊長不能先自行廣泛讀取、實作、審查或驗證，再事後補團隊軌跡。
- 完成宣稱必須回收 implementation change delivery、memory/docs delivery、review、validation 與 Team-Native trace。
- 缺任一合格交付件或 trace 時，只能標示阻塞、未驗證或總監風險關閉但非完整。

### 總監可讀輸出

AI_Rules 的面向總監輸出採情境式格式：一般討論、狀態回報與簡短判斷可用短段落或短清單；正式計畫、寫入前風險、多檔案變更、完成報告、健檢報告與交接才需要表格或結構化摘要。
若使用表格，欄位統一為「事項、位置、影響、狀態」。其中「位置」欄不得只寫概念詞，必須先寫白話位置，再用括號標出具體檔案、區塊、工具狀態或目錄範圍；若不是單一檔案，也要明說它是工作區狀態、工具結果或目錄範圍。

正式輸出若為了可讀性使用「核心規範、工作流入口、文件說明、巡檢規則、記憶卡」這類短名稱，必須在同一份輸出提供「位置索引」，把每個短名稱對應到具體檔案、章節、工具狀態或目錄範圍。
技術細節仍可保留，但只在必要時放入「補充技術細節」段落。
三平台的工作流與指令入口皆需明示此契約，避免只靠核心規則造成平台落差。

面向總監的文字也套用「技術詞彙翻譯閘門」：函式名稱、變數名稱、欄位名稱、命令參數、內部工具名或檔案路徑不可裸露出現。
每一次提到時，都必須先寫白話名稱，再把技術名稱放在括號內，例如「建構流程規則（03-build-建構/SKILL.md）」。
技術名稱不得單獨成為句子的主詞、清單項目或表格值；後續再次提到同一項目時，也要維持同一個白話名稱與括號定位。

### 中立誠實協作與知識新鮮度

AI_Rules 要求 AI 保持中立誠實協作。
AI 不以討好、附和或迎合操作者為目標，也不得為了顯得有批判性而刻意反對。
AI 必須先承接目標，再依實際檔案、工具輸出、官方文件或可靠主要來源檢查提議中的事實、日期、版本、限制與風險假設。
提議合理時明確支持；若證據與提議衝突，AI 要用「我看到的事實／可能問題／建議做法」短格式說明，並提出可行替代做法。

記憶卡與 AI 內建知識都視為可能過時。
本地檔案與即時工具輸出優先於記憶；官方文件與主要來源優先於模型印象。
遇到外部框架、API、套件版本、平台規則、價格、法規、安全建議、近期狀態或 AI 自己不確定的資訊，必須先查最新或官方來源；若無法查證，要明說尚未查證最新狀態，不能把舊記憶當成確定事實。
...
| 工作流外部接地矩陣 | 見下方位置索引 | 08 以外工作流共用任務型態、外部依據、最低證據與路由 |
...
| 平台能力矩陣 | 見下方位置索引 | 標記 native、adapter、conditional、unavailable 與 manual 路由 |
| Team-Native Core 政策 | 見下方位置索引 | 定義站點優先規則、狀態機、平台 adapter 邊界、完成條件與 trace |
| Team-Native trace 契約 | 見下方位置索引 | 定義任務軌跡欄位與稽核語意，且軌跡不直接寫入來源記憶 |
| 隊長制編程治理主幹 | 見下方位置索引 | 定義觸發、隊長最小執行權、角色互斥、主線直做例外與完成誠實性 |
| 團隊任務板模板 | 見下方位置索引 | 保存任務板、專員指派、交付件、逐波派工、風險關閉與收尾檢查表 |
| 團隊協作子技能 | 見下方位置索引 | 拆分角色邊界、變更、記憶、驗證、審查與完成閘門 |
| 子代理治理模型 | 見下方位置索引 | 定義 Delegation Gate、evidence branch、隔離交付、角色互斥與假團隊防線 |
...
位置索引：
- 工作流外部接地矩陣：來源 `Shared/workflow-capability-evidence-matrix.md`，部署到 `.agents/shared/`。
- 平台能力矩陣：來源 `Shared/platform-capability-matrix.md`，部署到 `.agents/shared/`。
- Team-Native Core 政策：來源 `Shared/policies/team-native-core.md`，部署到 `.agents/shared/policies/`。
- Team-Native trace 契約：來源 `Shared/policies/team-trace-evidence.md`，部署到 `.agents/shared/policies/`。
- 隊長制編程治理主幹：來源 `Shared/skills/programming-team-governance/SKILL.md`，部署到 `.agents/skills/`。
- 團隊任務板模板：來源 `Shared/skills/team-task-board/SKILL.md`，部署到 `.agents/skills/`。
- 團隊協作子技能：來源 `Shared/skills/team-*`，部署到 `.agents/skills/`。
- 子代理治理模型：來源 `Shared/policies/subagent-invocation.md`，部署到 `.agents/shared/policies/`。
...
graph TB
    ...
    subgraph "外部 MCP 依賴"
        GW["Multi-MCP Gateway"]
        CART["cartridge-system"]
        GH["github"]
        GN["gitnexus"]
        OTHER["外部 MCP 工具"]
    end
...
- Claude 源碼：`Claude/`，Claude Code 版源碼。
- Codex 源碼：`Codex/`，OpenAI Codex 版源碼。
- 統一部署引擎：`Scripts/Deploy.ps1 -Platform {Antigravity|Claude|Codex}`，支援 Fresh、Upgrade、Audit、Sync。
- Antigravity runtime：`.agents/`，包含 rules、workflows 與 62 套 skills。
- Claude runtime：`.claude/`，包含 rules、commands 與 62 套 skills。
- Codex runtime：`.codex/` 加 `.agents/skills/`，包含治理規則與 79 套 skills。
- 共用記憶庫：`.agents/memory/`，三平台共用來源記憶。
- 共用專案脈絡：`.agents/context/`，三平台共用設計 DNA 與長期偏好。
- Multi-MCP Gateway：以 `gateway__call_tool` 呼叫下游工具，需帶 `workspace` 與 `projectRoot`。
- 外部 MCP 工具：包含 cartridge-system、github、gitnexus、cloudflare、supabase、sentry、excel 等。
...
| **子代理人：啟動** | 隊長任務板建立後 | 隊長任務板建立後 | 隊長任務板建立後 |
| **子代理人：通道 A** | Gemini CLI / `@` / browser-capable agent | description-driven subagent / `@agent` | Codex native subagents |
| **子代理人：通道 B** | Antigravity plugin adapter | governed `Agent(...)` | 專案自訂代理 |
| **子代理人：交付** | 受治理隔離變更交付 | 受治理隔離變更交付 | 受治理隔離變更交付 |
| **子代理人：不可用** | 未驗證、阻塞或總監風險關閉但非完整 | 未驗證、阻塞或總監風險關閉但非完整 | 未驗證、阻塞或總監風險關閉但非完整 |
| **子代理人：Codex 限制** | — | — | Director 要求只會強制建板派工，不允許先開代理 |
...
| **技能總數** | 62 套 | 62 套 | **79 套**（62 共用 + 17 工作流） |

---

## 🔗 三平台共用記憶系統

三個平台安裝在同一個專案時，共享位於 `.agents/memory/` 的原始碼記憶卡。
原始碼記憶卡透過外部依賴 [Multi-MCP Gateway](https://github.com/Kunshao1117/Multi-MCP) 呼叫 cartridge-system。
[cartridge-system](https://github.com/Kunshao1117/cartridge_system) 作為統一讀寫引擎。
長期偏好與設計 DNA 則共享於 `.agents/context/`，不經過 `memory_commit`。
...
內容品質標準把記憶卡升級為可審計的專案事實治理層。
新建或受控標準化後的作用中主卡必須標記內容品質版本、記憶類型、驗證狀態、最後驗證時間與有效範圍，並補上證據基礎、讀取契約、衝突與取代段落。
偏好、設計 DNA、一次性觀察與未驗證推論不得寫入來源記憶；應分別留在專案脈絡、任務報告或待審清單。

記憶卡是來源記憶，不是可執行技能。
目標標準主檔為 `MEMORY.md`；既有專案仍可能在相容期保留 `.agents/memory/**/SKILL.md`。
正式更名必須等 cartridge-system 支援確認後，透過部署後專案本地工具乾跑、檢查衝突，再另行授權套用。
本專案目前已完成記憶卡主檔更名。
...
框架源碼中的共用脈絡模板位於 `Shared/context/`。
部署到目標專案時，初始化流程只會在缺少 `.agents/context/_map/CONTEXT.md` 時從模板建立索引。
若目標專案已有脈絡卡，Fresh、Upgrade、Sync 與孤兒清理都必須保留，不能覆蓋、合併或刪除。
...
| [cartridge-system](https://github.com/Kunshao1117/cartridge_system) | 記憶卡讀寫引擎；工具細節見下方 |

cartridge-system 外部依賴包含：
- `memory_list`、`memory_read`、`memory_status`、`memory_deps`、`memory_audit`。
- `workspace_brief`、`commit_preflight` 與高風險寫入工具 `memory_commit`。
...
│   └── skills/                        ← 62 套操作型技能唯一真實來源，部署時注入各平台
│
...
│       ├── Audit.psm1                 ← Audit facade / loader / public exports
│       └── Audit/                     ← Audit internal partials（00.Common.ps1 → 99.PlatformGovernanceAudit.ps1）
│
...
│       ├── skills/                    ← 62 套操作型技能（部署時從 Shared/ 注入）
...
| **隊長制編程治理覆蓋審計** | Doctor 檢查範圍與正式證據資格見下方覆蓋細節 |
| **Team-Native Core 審計** | 治理巡檢 Doctor 會掃描團隊原生核心政策、任務軌跡契約、平台能力路由、工作流證據矩陣與部署後共用參考；必要時可要求任務軌跡，缺少 Team-Native trace 會標示未驗證或阻塞 |
| **Experiment 例外** | `03-1_experiment` 是受治理沙盒 workflow；沙盒條件、捷徑與升級條件見下方 |
| **孤兒檔案偵測** | 自動偵測源碼已刪除但目標仍存在的殘留檔案 |
| **孤兒清除選項** | 加入 `-RemoveOrphans` 參數可自動清除，預設僅標記 |
| **Runtime drift 巡檢** | 檢查使用者層全域規則是否與 repo source 同步；CRLF/LF 不算漂移 |
| **Shared policy drift 巡檢** | 檢查三平台子代理 marker block、來源政策與下游 deployed copy |
...
| **衍生技能自動補建** | 部署或專案同步會補建 discovery 連結；實體 `project-*` 目錄會被 Doctor 攔截 |

覆蓋細節：
- Doctor 掃描隊長制編程治理技能、團隊任務板模板、子代理政策、委派策略、能力矩陣與工作流證據矩陣。
- Doctor 也掃描受治理請求觸發語意、05 濃縮寫入入口與三平台編程入口。
- Doctor 確認任務類型閘門、派工前置閘門、隊長最小執行權、隊長任務板與角色邊界。
- Doctor 確認 station-owned main-worktree change delivery 與 fallback change-application。
- Doctor 確認證據負責人、主線直做例外、全主線假團隊防線與部署副本一致。
- Doctor 檢查 formal-write 派工生命週期、逐波次派工、上一波輸入、下一波啟動條件與正式證據資格。
- Doctor 攔截草案板啟動隊員、草案證據冒充正式驗收、正式板後一次開全部隊員。
- `03-1_experiment` 可由 03-1、experiment、sandbox prototype、spike 或髒碼原型請求觸發。
- Experiment 使用 reduced/minimal experiment station/board，但仍需記錄 sandbox scope 與允許改動範圍。
- Experiment 也需記錄丟棄條件、升級條件、allowed shortcuts、experiment-only 處置與 scope-bound authorization。
- Sandbox writes、discard 與 promotion 不等於 production completion。
- Promotion to 03/build 或 production source/governance/public-contract write 需要新的 production scope-bound authorization。
- Production promotion 也需要 `formal-write`、station-owned `change-delivery`、validation、review 與 memory/docs。
- Production promotion 不可標為 automation-safe。
- Runtime drift 會檢查 `~/.codex/AGENTS.md`、`~/.claude/CLAUDE.md`、`~/.gemini/GEMINI.md` 是否同步。
- Shared policy drift 會比對 `Shared/policies/subagent-invocation.md` 與下游 `.agents/shared/policies/` 副本。
- 衍生技能補建會建立 `.agents/skills/project-*` 與 `.claude/skills/project-*` discovery 連結。
...
部署到專案後，Antigravity 使用 `.agents/VERSION`，Claude 使用 `.claude/VERSION`，Codex 使用 `.codex/VERSION`。
`.agents/` 雖同時承載 Codex skills 與三平台共用記憶，但 `.agents/VERSION` 只代表 Antigravity，不代表 Codex。

### 操作型技能同步原則

62 套操作型技能存放於 `Shared/skills/`（唯一真實來源），部署時由 `Scripts/modules/Skills-Sync.psm1` 注入各平台：
...
`Shared/skill-governance.md` 定義 Skill 放置與觸發契約。
核心規則只保留不可漏掉的安全底線，workflow / command 負責入口與階段判斷。
Shared Skill 承載可按需載入的操作細節。
memory 只記錄原始碼事實與決策，context 承載長期偏好與設計 DNA。
Doctor 的技能品質檢查會同步檢查 `description` 觸發品質，要求 Shared Skill 具備中英觸發詞與負向邊界，workflow / command 入口也要描述何時啟動，避免格式正確但 AI 不會自動讀取。

### 共用治理參考同步原則

下游專案不會取得完整 `Scripts/` 或 `Shared/` 來源樹。
部署與專案同步會把 AI 執行規則所需的唯讀治理參考複製到 `.agents/shared/`。
內容包含平台能力矩陣、工作流證據矩陣、技能治理規格、子代理政策與 MCP opt-in 片段。
部署也會把受限的專案本地工具複製到 `.agents/tools/`，目前包含記憶主檔遷移乾跑與受控套用工具。
同步套用、孤兒清理與來源更新等管理器級操作仍必須透過 AI Rules Manager 來源倉庫或 VS Code 外掛執行。

### 專案脈絡模板同步原則

共用脈絡模板存放於 `Shared/context/`，目前提供 `_map/CONTEXT.md` 作為所有目標專案的預設索引卡來源。
部署引擎只在目標專案缺少索引卡時複製模板；已存在的 `.agents/context/` 一律視為專案知識資產並保護。
治理巡檢會同時檢查共用模板是否存在，以及目標專案脈絡卡格式是否符合協議。

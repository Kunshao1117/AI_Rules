...
3. **角色權限模糊** — 子代理人隨意修改檔案，或 AI 形式上列站點卻仍全部主線直做。→ 編程意圖自動進入隊長制團隊模式，先建立隊長任務板；證據型站點預設交給唯讀證據分支，實作隊員只能在受治理隔離區產出補丁，審查者不能實作同一交付物；所有主工作區寫入與交付仍由隊長整合
4. **知識碎片化** — 技能散落各處，Token 消耗暴增。→ 43 套按需載入的操作型技能，不用時零開銷
...
|------|---------|---------|--------|--------|-----------|---------| 
| **Antigravity** | Gemini（IDE 插件 + CLI） | v8.0.3 | 9 | 20 | 43 | [Antigravity/README.md](Antigravity/README.md) |
| **Claude Edition** | Claude Code（VS Code 插件） | v1.2.3 | 7 | 17 | 43 | [Claude/README.md](Claude/README.md) |
| **Codex Edition** | OpenAI Codex（agentskills.io 標準）| v0.1.3 | 1 | 17 | 43 | [Codex/README.md](Codex/README.md) |

三個版本的**操作型技能均源自 `Shared/skills/`**（唯一真實來源，43 個），記憶系統依賴 [cartridge-system](https://github.com/Kunshao1117/cartridge_system)，並透過 [Multi-MCP Gateway](https://github.com/Kunshao1117/Multi-MCP) 統一探索與呼叫下游 MCP 工具（外部依賴）。專案脈絡層使用 `.agents/context/` 保存設計 DNA、產品偏好、技術偏好、溝通偏好與驗收偏好，不走記憶 stale。
...
| **隊長制編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入隊長制，不要求總監手動指定工作流；每個站點明確標示是否適用、執行模式、證據負責人、角色邊界、主線直做例外與完成條件；證據型站點預設交給唯讀團隊採證，實作隊員只能產出隔離補丁，不能自我審查 |
...
| **最小權限治理** | 角色分層（讀取者 / 工作者 / 寫入者），子代理啟用政策由 `Shared/policies/` 同源轉譯；子代理只能做唯讀採證、反證、測試策略與審查意見，不能寫檔、提交、推送、發布或改記憶；缺少分支證據時必須標示未驗證、阻塞或具體主線直做例外 |
...
| 隊長制編程治理主幹 | 來源：`Shared/skills/programming-team-governance/SKILL.md`；部署：`.agents/skills/programming-team-governance/SKILL.md` | 以隊長觸發閘門、隊長任務板、角色互斥、隔離補丁、需求回放、反證、影響面、計畫授權、實作、短迴圈、審查、收尾取代「必要時」與大小型模糊判斷；每站必須分開標示是否適用、執行模式、證據負責人、角色邊界、主線直做例外與完成條件 |
...
graph TB
    ...
    subgraph "目標專案（三版本可共存）"
        ...
        CXE[".codex/ + .agents/skills/<br/>治理規則 + 技能（60 套）"]
...
| **子代理人** | 隊長站點 → Gemini CLI / `@` 指派 / browser-capable agent / Antigravity plugin adapter / 受治理隔離補丁；不可用時標記未驗證、阻塞或主線直做例外 | 隊長站點 → description-driven subagent / `@agent` / governed `Agent(...)` / 受治理隔離補丁；不可用時標記未驗證、阻塞或主線直做例外 | 隊長站點 → Director 要求、工作流站點要求或 `.codex/agents/*.toml` / 受治理隔離補丁；不可用時標記未驗證、阻塞或主線直做例外 |
...
| **技能總數** | 43 套 | 43 套 | **60 套**（43 共用 + 17 工作流） |
...
│   └── skills/                        ← 43 套操作型技能唯一真實來源，部署時注入各平台
│
...
| **隊長制編程治理覆蓋審計** | 治理巡檢 Doctor 會掃描隊長制編程治理技能、子代理政策、委派策略、能力矩陣、工作流證據矩陣、00/01 觸發轉向與三平台編程入口，確認隊長任務板、角色邊界、隔離補丁、證據負責人、主線直做例外、全主線假團隊防線與部署副本一致 |
...
43 套操作型技能存放於 `Shared/skills/`（唯一真實來源），部署時由 `Scripts/modules/Skills-Sync.psm1` 注入各平台：

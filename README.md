...
3. **角色權限模糊** — 子代理人隨意修改檔案，主代理人無法審閱。→ 編程任務先建立團隊站點板，子代理只做唯讀證據分支，所有寫入與交付仍由主代理整合
...
| **編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造都先建立團隊站點板；每個站點明確標示主線直做、唯讀隊員採證、阻塞或不適用 |
...
| **最小權限治理** | 角色分層（讀取者 / 工作者 / 寫入者），子代理啟用政策由 `Shared/policies/` 同源轉譯；子代理只能做唯讀採證、反證、測試策略與審查意見，不能寫檔、提交、推送、發布或改記憶 |
...
| 編程團隊治理主幹 | 來源：`Shared/skills/programming-team-governance/SKILL.md`；部署：`.agents/skills/programming-team-governance/SKILL.md` | 以需求回放、反證、影響面、計畫授權、實作、短迴圈、審查、收尾八站取代「必要時」與大小型模糊判斷；每站必須分開標示是否適用與執行模式 |
| 子代理治理模型 | 來源：`Shared/policies/subagent-invocation.md`；部署：`.agents/shared/policies/subagent-invocation.md` | Shared 只定義 Delegation Gate 與 evidence branch 語義；Codex、Claude、Antigravity 各自由 adapter 轉成平台工具 |
...
| **編程團隊治理覆蓋審計** | 治理巡檢 Doctor 會掃描編程團隊治理技能、子代理政策、委派策略、能力矩陣、工作流證據矩陣與三平台編程入口，確認固定站點板、唯讀隊員邊界與部署副本一致 |
| **Experiment 例外** | `03-1_experiment` 是刻意設計的沙盒例外，允許快速寫檔並跳過正式品質、測試與記憶收尾，但仍必須先列最小團隊站點、沙盒邊界、允許改動範圍、丟棄條件與升級條件，且不可標為 automation-safe |

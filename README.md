...
3. **角色權限模糊** — 子代理人隨意修改檔案，或 AI 形式上列站點卻仍全部主線直做。→ 編程意圖自動進入隊長制團隊模式，先建立隊長任務板；證據型站點預設交給唯讀證據分支，實作隊員只能在受治理隔離區產出補丁，或在無隔離時產出文字補丁任務包，審查者不能實作同一交付物；所有主工作區寫入與交付仍由隊長整合
4. **知識碎片化** — 技能散落各處，Token 消耗暴增。→ 44 套按需載入的操作型技能，不用時零開銷
...
|------|---------|---------|--------|--------|-----------|---------| 
| **Antigravity** | Gemini（IDE 插件 + CLI） | v8.0.3 | 9 | 20 | 44 | [Antigravity/README.md](Antigravity/README.md) |
| **Claude Edition** | Claude Code（VS Code 插件） | v1.2.3 | 7 | 17 | 44 | [Claude/README.md](Claude/README.md) |
| **Codex Edition** | OpenAI Codex（agentskills.io 標準）| v0.1.3 | 1 | 17 | 44 | [Codex/README.md](Codex/README.md) |

三個版本的**操作型技能均源自 `Shared/skills/`**（唯一真實來源，44 個），記憶系統依賴 [cartridge-system](https://github.com/Kunshao1117/cartridge_system)，並透過 [Multi-MCP Gateway](https://github.com/Kunshao1117/Multi-MCP) 統一探索與呼叫下游 MCP 工具（外部依賴）。專案脈絡層使用 `.agents/context/` 保存設計 DNA、產品偏好、技術偏好、溝通偏好與驗收偏好，不走記憶 stale。
...
| **隊長制編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入隊長制，不要求總監手動指定工作流；總監只對隊長說話，隊長先判斷任務類型、工作流路由、實作授權、允許角色與禁止角色，再依團隊任務包模板建立隊長任務板；任何隊員或分支不得早於任務板啟動；隊長派一任務一隊員，實作隊員只產出隔離或文字補丁包，審查與驗證由不同隊員互驗，隊長最後只整合已回收且可追溯的補丁與證據，並保留主線寫入、審查狀態、記憶、提交、發布與最終驗收 |
...
graph TB
    ...
    subgraph "目標專案（三版本可共存）"
        ...
        CXE[".codex/ + .agents/skills/<br/>治理規則 + 技能（61 套）"]
...
| **技能總數** | 44 套 | 44 套 | **61 套**（44 共用 + 17 工作流） |
...
│   └── skills/                        ← 44 套操作型技能唯一真實來源，部署時注入各平台
│
...
| **隊長制編程治理覆蓋審計** | 治理巡檢 Doctor 會掃描隊長制編程治理技能、團隊任務包模板、子代理政策、委派策略、能力矩陣、工作流證據矩陣、00/01 觸發轉向、05 濃縮寫入入口與三平台編程入口，確認任務類型閘門、派工前置閘門、隊長最小執行權規則、隊長任務板、角色邊界、隔離補丁、文字補丁、證據負責人、主線直做例外、全主線假團隊防線與部署副本一致 |
...
44 套操作型技能存放於 `Shared/skills/`（唯一真實來源），部署時由 `Scripts/modules/Skills-Sync.psm1` 注入各平台：

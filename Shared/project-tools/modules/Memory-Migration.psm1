...
<#
.SYNOPSIS
    Memory card main-file naming migration helper for deployed projects.
.DESCRIPTION
    ...
    when explicitly run with -Apply and -ConfirmApply. Archive volumes are never
    renamed and body references are reported only.
#>
...
function Write-MemoryMainFileMigrationReport {
    ...
    if (-not $Apply) {
        Write-Host ""
        Write-Host "Dry-run：未指定套用參數，不會更名任何記憶卡。" -ForegroundColor Yellow
        Write-Host "注意：本工具只回報舊路徑引用，不批次修正文內引用。"
    }
}
...
function Invoke-MemoryMainFileMigration {
    ...
    .PARAMETER ConfirmApply
        Required with Apply for project-local use. Manager wrappers may pass it after their own confirmation.
    .PARAMETER WhatIf
    ...
    if (-not $ConfirmApply) {
        throw "記憶主檔遷移已停止：套用更名必須同時指定 -Apply 與 -ConfirmApply，且需先取得總監授權。"
    }

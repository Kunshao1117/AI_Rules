...
範圍式授權是三平台共用語義。任何人類文字、介面按鈕、模式切換、權限提示、工作流指令或工具確認，都必須先解析成「授權誰、做哪個階段、哪個站點、哪批檔案、哪個範圍、哪個期限、哪個命令或哪個工具呼叫」。GO、繼續、回去修正、做剛剛那個、按鈕核准，只能綁定目前可見提示、計畫、差異、命令、站點、檔案清單、範圍、階段、期限或 Team-Native station trace。若範圍無法判定，該訊號只能算路由意圖或部分證據，不得當成寫入授權。

| 授權訊號 | 共用語義 | 不授權項目 |
|------|------|------|
| 總監文字同意 | 只授權上一個明確計畫、站點、檔案清單、命令、範圍、階段、期限或工具呼叫；模糊同意需綁定最近的具體範圍 | 未列出的檔案、順手重構、記憶、提交、推送、發布、部署、安裝、憑證或外部狀態 |
| 介面按鈕同意 | 可作為授權證據，但必須保留按鈕所屬提示、動作、差異、命令、站點、檔案範圍、階段與期限 | 介面沒有顯示的隱藏寫入、跨站點工作、無範圍批次變更 |

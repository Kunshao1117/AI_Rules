...
| Captain-led programming governance | `adapter`：自然語言編程任務與明示工作流都先建立隊長任務包；以隊長制觸發門、任務類型閘門、派工前置閘門、隊長最小執行權與 `team-task-package` 團隊站點板約束 Antigravity workflow；證據型站點預設轉譯為瀏覽器、CLI、插件或唯讀 specialist adapter；隔離補丁需平台 adapter 明確支援，無隔離時可退為文字補丁任務包 | `native` + `adapter`：自然語言編程任務與明示工作流都先建立隊長任務包；以隊長制觸發門、任務類型閘門、派工前置閘門、隊長最小執行權與 `team-task-package` 團隊站點板約束 Claude command；證據型站點預設轉譯為 Claude subagents、hooks 或隔離命令證據；隔離補丁需受控 workspace/checkpoint，無隔離時可退為文字補丁任務包 | `native` + `adapter`：自然語言編程任務與明示工作流都先建立隊長任務包；以隊長制觸發門、任務類型閘門、派工前置閘門、隊長最小執行權與 `team-task-package` 團隊站點板約束 Codex workflow；證據型站點預設轉譯為 Codex native subagents、瀏覽器、終端、背景任務或 MCP 主線工具；隔離補丁需 forked workspace 或等價安全邊界，無隔離時可退為文字補丁任務包 |
| Subagents | `adapter`：Shared evidence branch / isolated patch / text patch 語義在隊長任務板建立後轉譯為 Gemini CLI subagents、`@` 指派、browser-capable agent 或 Antigravity plugin adapter；不可用時標記未驗證、阻塞或隊長代工風險接受 | `native`：Shared evidence branch / isolated patch / text patch 語義在隊長任務板建立後轉譯為 Claude Code built-in/custom/plugin subagents、description 自動委派、`@agent` 或 governed `Agent(...)` 權限模型；不可用時標記未驗證、阻塞或隊長代工風險接受 | `native`：Shared evidence branch / isolated patch / text patch 語義在隊長任務板建立後轉譯為 Codex native subagents；Director 要求只會強制建板派工，不允許先開代理；不可用時標記未驗證、阻塞或隊長代工風險接受 |
...
子代理治理語義以 `Shared/policies/subagent-invocation.md` 為唯一來源，隊長制編程團隊語義以 `Shared/skills/programming-team-governance/SKILL.md` 為唯一來源，任務板與專員任務包模板以 `Shared/skills/team-task-package/SKILL.md` 為唯一來源。Shared 層只描述 captain trigger gate、team station board、Delegation Gate、read-only evidence branch、isolated patch branch、role exclusivity、主代理整合責任、主線直做例外與回報格式；平台專用工具名稱只能出現在政策檔的平台轉譯區塊、平台專屬 workflow / command，或明確標示為對照的文件段落。

證據型站點是隊長制團隊協作主幹，不是加分選項。自然語言編程任務與明示工作流都必須先形成隊長任務包；工作流指令只是路由，不是免建板、免派工或免驗證的授權。隊長任務板必須先於任一隊員啟動，且必須記錄任務類型、工作流路由、實作授權、允許角色與禁止角色。隊長採最小執行權：保留總監溝通、GO 判讀、主工作區整合、記憶/提交/發布閘門與最終驗收；反證、影響面、測試、審查與收尾應優先由可分離站點採證。

一隊員只承接一個具體站點任務。實作隊員只能在受治理隔離邊界內產出補丁包，或在無檔案隔離能力時產出文字補丁任務包；不能直接改主工作區、不能更新記憶、不能提交，也不能審查自己的補丁。測試、審查與收尾隊員必須互驗補丁、驗證證據、偏移與未完成項。隊長只保留總監溝通、GO 解讀、已回收補丁包的主工作區整合、受保護狀態動作、審查狀態裁決與最終交付。正式整合需要補丁包、審查包、驗證包齊全；任一缺失時必須標示阻塞、未驗證或已接受風險。若缺少可用隊員、缺少隔離補丁能力、缺少文字補丁任務包，或所有證據站點都退回隊長代工，流程必須標示為阻塞、未驗證或隊長代工的已接受風險；不得宣稱完整團隊完成。若兩個以上證據型站點適用卻全部標為主線直做，必須逐站留下具體例外與替代證據；否則該流程只能標示為未驗證或阻塞。實作與審查角色不得自審；隔離補丁與文字補丁不得直接污染主工作區。

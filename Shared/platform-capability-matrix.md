...
| Captain-led programming governance | `adapter`：以隊長制觸發門與團隊站點板約束 Antigravity workflow；證據型站點預設轉譯為瀏覽器、CLI、插件或唯讀 specialist adapter；隔離補丁需平台 adapter 明確支援 | `native` + `adapter`：以隊長制觸發門與團隊站點板約束 Claude command；證據型站點預設轉譯為 Claude subagents、hooks 或隔離命令證據；隔離補丁需受控 workspace/checkpoint | `native` + `adapter`：以隊長制觸發門與團隊站點板約束 Codex workflow；證據型站點預設轉譯為 Codex native subagents、瀏覽器、終端、背景任務或 MCP 主線工具；隔離補丁需 forked workspace 或等價安全邊界 |
| Subagents | `adapter`：Shared evidence branch / isolated patch 語義轉譯為 Gemini CLI subagents、`@` 指派、browser-capable agent 或 Antigravity plugin adapter；不可用時標記未驗證、阻塞或具體直做例外 | `native`：Shared evidence branch / isolated patch 語義轉譯為 Claude Code built-in/custom/plugin subagents、description 自動委派、`@agent` 或 governed `Agent(...)` 權限模型；不可用時標記未驗證、阻塞或具體直做例外 | `native`：Shared evidence branch / isolated patch 語義轉譯為 Codex native subagents；在 Director 明確要求、coding workflow station 指定 branch，或 `.codex/agents/*.toml` custom agents 已配置時啟動；不可用時標記未驗證、阻塞或具體直做例外 |
...
| 委派邊界 | 子代理與瀏覽器分支限於唯讀採證，隔離補丁分支不得直接改主工作區；證據型站點不得全數假裝主線完成，主代理保留團隊站點裁決與整合責任 | 子代理、鉤子與檢查點不得取代主代理決策、團隊站點裁決與 GO gate；證據型站點需有分支證據或主線直做例外；實作與審查角色不得自審 | Codex 子代理在總監要求、工作流站點要求或專案代理配置時啟動；證據型站點需有分支證據或主線直做例外，隔離補丁必須由主代理合入，主代理保留團隊站點裁決 |
| 缺證處理 | 工具不可用時列出搜尋範圍、替代路徑與阻塞條件 | 權限、憑證或鉤子不可用時標記未驗證或阻塞 | 缺工具、缺沙盒、缺登入或未授權時標記未驗證或阻塞 |

## Shared Subagent Invocation Policy

子代理治理語義以 `Shared/policies/subagent-invocation.md` 為唯一來源，隊長制編程團隊站點語義以 `Shared/skills/programming-team-governance/SKILL.md` 為唯一來源。Shared 層只描述 captain trigger gate、team station board、Delegation Gate、read-only evidence branch、isolated patch branch、role exclusivity、主代理整合責任、主線直做例外與回報格式；平台專用工具名稱只能出現在政策檔的平台轉譯區塊、平台專屬 workflow / command，或明確標示為對照的文件段落。

證據型站點是隊長制團隊協作主幹，不是加分選項。若兩個以上證據型站點適用卻全部標為主線直做，必須逐站留下具體例外與替代證據；否則該流程只能標示為未驗證或阻塞。實作與審查角色不得自審；隔離補丁不得直接污染主工作區。
...
| 層級 | 可以使用的語彙 | 不應出現的語彙 |
|------|----------------|----------------|
| Shared 共用語義 | Captain Trigger Gate、Captain Team Board、team station、Role Exclusivity、Delegation Gate、evidence branch、isolated patch、browser branch、CLI branch、MCP direct、Master Agent | 未標註平台的 Claude Agent call syntax、舊 browser subagent token、舊 browser agent token、Codex native spawn helper token |

...
Team-Native Core 是三平台共同遵守的隊長制團隊語意：總監指令 -> 隊長接收 -> 轉譯 -> 建板 -> 分派專家子技能 -> 專家工作 -> 隊長監督 -> 回收變更交付件/證據交付件 -> 隊長接收並更新任務板 -> 已授權變更套用站點 -> 獨立驗證、審查與記憶文件站點 -> 完成審計 -> 回報。隊長只負責派工、接收站點交付、彙整狀態、處理阻塞與授權邊界；正式檢查、變更套用、驗證、審查與記憶文件判讀由對應站點負責。Team-Native / subagent team mode 對適用任務預設開啟；平台能力只決定執行通道，不決定是否啟動 team mode。它是治理核心，不代表每個平台都有同等原生子代理能力。

子代理、瀏覽器、CLI、MCP、隔離工作區與文字交付路徑都是執行通道。角色與責任必須先由 `team-specialist-registry` 與對應的 `team-specialist-*` 子技能決定，再映射到可用執行通道。

Team-Native Core 對唯讀探索、架構藍圖、廣泛讀檔、外部研究、影響面分析與驗證規劃同樣有效，只要這些結果可能影響後續 source、workflow、validation、review、memory、release 或 governance 決策。沒有寫入授權時使用 `formal-readonly`；已解析範圍且具備授權狀態的寫入才使用 `formal-write`。No-write does not mean no-team；no-write 是動作限制，不是 no-team。
...
正式團隊模型的拆解順序固定為：任務板 -> 站點族群 -> 正式站點 -> 子站點任務 -> 隊員配置 -> 執行通道 -> 交付件。站點族群用來保留需求、影響面、實作、驗證、審查、記憶文件、完成稽核等責任分區；正式站點是授權與證據資格的單位；子站點任務是可由單一隊員承接的最小工作包；隊員配置只決定每個子站點任務需要幾個隊員或通道；執行通道只負責讓該隊員工作；交付件才是可回收、可審查、可供隊長彙整狀態的證據，或可交由具名 station-owned change-application gate 處理的變更物。
...
正式實作不以主代理直做為正常路徑。實作站點預設是隔離變更交付件；無隔離時退為文字變更交付件；兩者都無法產出時預設標示 blocked。變更套用預設屬於 station-owned `change-application` 站點，該站點可在 formal-write、authorization phase `change-application`、精確檔案 allowlist、dirty diff read 與禁止受保護動作成立時寫入主工作區 source。隊長只能接收交付件、維持任務板、處理阻塞/衝突與授權；只有平台無法委派實體寫入或受保護工具呼叫時，才可進入 protected captain gate，且必須記錄 direct exception。隊長不得重寫、改寫或主要實作。隊長替代創作、代工或自行產出站點交付件，只能在總監逐案明示接受該缺口時標示 `closed-with-director-risk`，且只能作為非完整風險關閉，不得宣稱 `complete`。
...
| `browser branch` | 需要 UI、DOM、截圖、瀏覽器互動或視覺驗證 | 只回傳瀏覽器證據、失敗項目與建議路由；若需要修改主工作區，必須回到變更站點或具名 station-owned authorized change-application gate |
...
| `station-owned authorized change-application gate` | 已有 returned artifact 或明確 source scope，且 formal-write 授權綁定到 `change-application` 階段 | 預設由隊員站點持有並可寫主工作區 source；隊長只監督與接收 receipt；若平台不可委派才改走 captain-owned protected gate |
...
1. 任務需要主工作區 source 寫入但缺少 station-owned `change-application` 授權；或任務需要修改記憶卡、雲端資源、PR、Issue、版本控制或部署狀態。
...
- 主代理不得把團隊站點板、最終審查狀態或完成聲明交給證據分支裁決；變更套用只能交給正式 `change-application` 站點或平台不可委派時的 protected captain gate。
...
主工作區 source 寫入屬於另一個 formal-write `change-application` 站點，預設由隊員持有；若平台不可委派才可改走 protected captain gate。若平台沒有隔離機制且無法交付文字變更交付件，實作站點必須標示 blocked；只有總監逐案明示以 `closed-with-director-risk`（總監風險關閉但非完整）接受隊長替代創作或代工缺口時，才可由隊長產出替代內容並明確標示非完整，且不得把無法隔離視為正常 direct fallback 或完整完成證據。
...
正式團隊完成條件必須同時具備實作變更交付件、變更套用 receipt、記憶文件交付件、審查交付件與驗證交付件，必要交付件齊全才可宣稱 full team completion。變更交付件必須來自實作隊員且包含 `memory_impact`；變更套用必須由已授權的 station-owned `change-application` gate 依交付件範圍執行。protected captain gate 只可在平台不可委派時使用，且不得變成隊長改寫。隊長替代創作、代工、自行產出站點交付件、重寫或改寫隊員交付內容，只能在總監逐案明示風險關閉時標示 `closed-with-director-risk`（總監風險關閉但非完整），不能滿足正式團隊完成。記憶文件交付件必須包含 `memory_impact` 與 `memory_delivery`；審查交付件必須來自未參與實作的審查隊員；驗證交付件必須來自未修改核心實作的測試或驗證路徑。任一交付件缺失、缺獨立審查或缺驗證時，站點只能標示 `blocked`、`unverified` 或 `closed-with-director-risk`，不得宣稱完整團隊完成。需要可稽核完成時，還必須具備 Team-Native 軌跡證據，或把缺軌跡標為 unverified / blocked。

...
> [!CAUTION]
> 🟠 **系統強制攔截**：此記憶已過期失真！
> 追蹤檔案異動：`Claude/.claude/commands/07_debug(除錯)/SKILL.md`、`Claude/.claude/commands/11_handoff(交接)/SKILL.md`（2026-06-04T03:26:01+08:00）
...
- **Claude 記憶卡壓縮治理 (2026-06-04)**: Claude README、記憶契約、禁用詞規則與 build/fix/debug/blueprint/handoff 指令已採用 schema v2 記憶語義，要求讀寫 Current Truth、Active Constraints、Cycle Events、Archive Index、中文摘要與 compaction status，舊卡可讀但在更新時走懶升級。
- **Claude 記憶路徑修正 (2026-06-04)**: Claude build command 與核心身份規則已改回三平台共用 `.agents/memory/`，不得再使用舊式 `.claude/agents/memory/` 路徑。
...
- **Claude Shared skill count refresh (2026-05-31)**: 新增 `ui-design-exploration` 後，Claude README 的 Shared 操作型技能數同步更新為 40 套；實際技能內容仍由 `Shared/skills/` 注入。

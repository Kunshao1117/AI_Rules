...
- Claude source and deployed rules now use `closed-with-director-risk` for Director-closed non-complete missing implementation-channel cases.
- Claude commands now load specialist-role skills and use change delivery artifact semantics for implementation stations; shared operational skill count is 61.
- Claude core rules now require the Director to talk to the captain only, one bounded task per role-exclusive specialist, no captain-authored primary implementation/review/validation/memory attribution, and four-delivery-artifact completion before full team completion.
- Claude command entries now load team-task-board as the team-board template source; Claude shared operational skill count is 61.
- Claude source and deployed core rules now share the updated subagent policy block for board-first captain-led dispatch.
- Claude coding commands require task type classification, dispatch pre-gate, Captain Minimum Execution Gate, text change delivery, and `closed-with-director-risk` for non-complete captain substitute authoring before any subagent or specialist branch starts.
...
- Claude shared operational skill count is 61.
## Active Constraints
...
- 24: Synced Claude source/deployed policy marker blocks with `closed-with-director-risk` wording.
- 23: Synced Claude source and deployed commands with Team-Native specialist registry and change delivery artifact terminology.
- 22: Synced Claude source and deployed core rules with Team-Native Core route-state policy.

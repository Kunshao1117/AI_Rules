...
description: >-
  ...
  scope.
last_updated: '2026-07-03T13:41:22+08:00'
status: stable
staleness: 0
...
- Formal Team-Native work is default for governance, workflow, fix/build/validation/review, memory/docs, commit/release/handoff, skill-forge, and public-contract tasks; narrow pre-board read-only probes remain orientation only.
- Team-Native Core owns team policy, route states, trace evidence, role boundaries, channel state, station-owned change application, non-complete states, and completion contracts in `_shared.team-native-core`.
- Language governance is the shared source for audience layers, exact-evidence preservation, Director-facing meaning-first output, field display, Chinese-first planning vocabulary, and read-only/write-stage wording separation.
- Memory governance separates source memory from project context; memory card writes, topology changes, context writes, migration/reindex, and `memory_commit` require separately authorized protected phases.
...
## 中文摘要
- Shared 是跨平台治理與共用技能來源；細部 Team-Native 與操作技能事實應交由子卡維護。

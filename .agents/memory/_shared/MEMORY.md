...
description: >-
  ...
  scope.
last_updated: '2026-06-28T14:08:50+08:00'
status: stable
staleness: 0
...
## Current Truth
- Shared governance separates draft boards from formal dispatch boards and requires wave-gated dispatch, previous-wave input, next-wave start condition, formal evidence eligibility, and no post-board all-at-once launch.
- Direct exceptions require no separable evidence scope plus explicit replacement evidence; size, speed, cost, tiny, and single-step wording is rejected as a direct route.
- Full team completion requires implementation patch, memory delivery, independent review, validation evidence, and completion evidence, or explicit blocked/unverified/accepted-risk status.
- team-task-package is the canonical source for captain board templates, specialist packets, evidence packets, patch packet types, direct exceptions, and completion checklists.
- Captain Trigger Gate and Captain Team Board route explicit workflows plus natural-language coding intent into the default team collaboration model; Shared now owns 50 operational skills.
...
- Shared governance references deployed to `.agents/shared/` include platform/workflow matrices, skill governance, subagent policy, MCP opt-in snippets, and 50 operational skills.
...
## Cycle Events
- 25: Verified final Doctor/Audit green after four-packet Doctor checks, compressed captain skills, and deployed skill sync.
- 24: Compressed captain/main delegation skills, updated Doctor four-packet checks, and resynced source/deployed policy markers.
- 23: Added formal team child-skill routing with implementation patch, memory delivery, review, and validation packets; refreshed 50/67 skill facts after source/deployed sync.
- 22: Added draft/formal board and wave-dispatch governance across shared matrices and policy, then verified Doctor/Audit green with entrypoint coverage.
- 21: Hardened team-task-package direct exceptions to require replacement evidence instead of small, tiny, or single-step wording.
- 20: Hardened shared policy, matrices, team-task-package, and delegation strategy around four-packet completion and verified Doctor/Audit green with source/deployed hashes.
- 19: Added team-task-package template governance, refreshed 50/67 skill-count facts, and verified Doctor/Audit green.
- 18: Replaced minimum-execution wording with text patch packets and captain substitution accepted-risk in shared governance memory.
- 17: Added task-type, dispatch pre-gate, and captain minimum-execution governance across shared matrices and policy.

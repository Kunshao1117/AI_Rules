...
description: >
  ...
  下任何共用技能或平台治理資產時。
scopePath: Shared/
last_updated: '2026-06-04T03:13:05+08:00'
staleness: 10
status: stale
metadata:
  author: antigravity
...
- **記憶卡壓縮治理 (2026-06-04)**: `memory-ops` 與 `memory-arch` 現行規則採用 schema v2 語義：英文 Current Truth、Active Constraints、Cycle Events、Archive Index、中文摘要，主卡上限 16 KB / 120 行，週期事件上限 30 筆，歸檔卷上限 32 KB / 200 行，舊卡採懶升級；source memory topology examples now describe current truth and constraints instead of legacy history containers.
- **記憶欄位一致性修正 (2026-06-04)**: `memory-ops` 的 legacy lazy upgrade 必填欄位清單已與新版模板對齊，包含 cycle_event_limit 與 line_limit，避免新卡與升級卡規格分叉。
- **Shared 技能讀取語義更新 (2026-06-04)**: `audit-engine`、`impact-test-strategy`、`test-patterns` 與 `memory-arch` 已改讀或描述 Current Truth / Active Constraints / Cycle Events / Archive Index，不再要求把修復過程無條件追加到舊式 Key Decisions、Known Issues 或 Module Lessons。
...
- **長期偏好不放進原始碼記憶**：設計 DNA、產品偏好與驗收偏好屬於專案脈絡層；原始碼記憶只保存 source ownership、Current Truth、Active Constraints、Cycle Events、Archive Index、Relations 與 staleness 等施工事實，避免偏好資訊混入 `memory_commit` 的過期判斷。

...
- Commit/release workflows require formal team evidence, team-task-board inheritance, programming-team-governance, delivery artifact separation, wave-gated evidence, applicability/execution-mode, evidence owner, direct exception, completion condition, and all-direct guard reporting.
- Protected memory, version-control, push, release, deploy, and install mutations route to separately scoped owner stations or platform-nondelegable protected-action records; review, validation, and completion stay separated evidence roles.
- Commit, routine, and skill-forge workflows require task type, dispatch pre-gate, Team-Native startup gate, text change delivery, `closed-with-director-risk`, and role-bound evidence branches before specialist work.
- MCP Memory Evidence Matrix supports commit preflight, read-only routine, and skill attribution evidence; commit workflow verifies memory state and git scope before external git state changes.
- This child card owns Codex commit, routine, and skill-forge workflow skills; routine stays automation-safe only while read-only, and commit/routine conclusions check review-state blockers, accepted-risk items, unverified high-risk validation, and review governance coverage.
...
- Do not collapse memory write into `memory_commit`; active-card edits, index/staleness sync, git operations, and release/deploy/install operations each require their own protected gate.
...
- 21: Recorded release-side workflow hardening so commit, routine, handoff, and skill-forge evidence uses formal team routing while protected mutations use owner-station or platform-nondelegable protected-action records.
...
- 15: Compressed legacy coordination delegation skills, updated Doctor four-delivery-artifact checks, and resynced source/deployed policy markers.
...
- 13: Updated Codex release workflow memory for formal dispatch board fields and protected release-completion owner-station gates.
- 12: Tightened Codex commit workflow around evidence-only specialists, owner-station git/memory/release actions, and four-delivery-artifact completion.
- 11: Added team-task-board template governance, refreshed 50/67 skill-count facts, and verified Doctor/Audit green.
- 10: Updated Codex release-side workflow memory for Team-Native minimum execution and text change delivery artifact governance.
- 9: Updated Codex release-side workflows for the new Team-Native dispatch gate.
- 08: Aligned Codex release-side workflows with role-bound evidence stations and owner-station mutation paths.
...
- `GO MEMORY WRITE` 不包含 `memory_commit`、git、release、deploy 或 install。

...
> [!CAUTION]
> 🟠 **系統強制攔截**：此記憶已過期失真！
> 追蹤檔案異動：`Claude/.claude/commands/11_handoff(交接)/SKILL.md`（2026-06-04T03:26:01+08:00）
...
- **D35: Claude 記憶卡壓縮契約 (2026-06-04)**: Claude 記憶契約與指令入口已對齊 schema v2：Current Truth / Active Constraints / Cycle Events / Archive Index / 中文摘要與 compaction status 為標準讀取重點，debug 與 handoff 不再抽取 legacy issue/decision fields；週期事件滿 30 筆或主卡過大時必須先彙整，舊卡採懶升級。
- **D36: Claude build memory path correction (2026-06-04)**: Claude build command and core identity now point memory updates to shared `.agents/memory/`; `.claude/agents/memory/` remains forbidden legacy storage.

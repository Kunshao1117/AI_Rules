...
- `classification`
  - Exactly one of `explicit-scope`, `minimal-implementation-detail`,
    `approval-required-expansion`, or `suggestion-only`. The category meanings
    and decision boundary are owned by `requirement-precision.md`.
  - Classification describes content only. `result` and `next_action` remain
    independent dispositions; a classification never replaces an existing

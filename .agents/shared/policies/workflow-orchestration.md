...
- It separately classifies the proposed work as `explicit-scope`,
  `minimal-implementation-detail`, `approval-required-expansion`, or
  `suggestion-only` through `requirement-precision.md`; the classification
  describes content, while the existing result and route remain the
  disposition. A category never creates authorization.

...
### feat
- **初學者介面與技術資料分層** — 側邊面板、指令名稱、狀態列、確認視窗、通知與更新提示改為白話繁體中文；完整命令、路徑、原始錯誤與診斷仍保留在技術資料。
- **統一結果轉換** — 新增單一顯示轉換模組，集中將管理工具輸出、已知錯誤與取消操作轉為「結果、影響、下一步」，避免每個按鈕各自判斷。

### fix
- **安全說明更清楚** — 工作區信任、自訂來源信任、管理快取重建、同步、清理與取消操作現在會明確說明影響、保留內容與不會修改的範圍。
- **技術輸出不再打擾一般操作** — 正常操作仍完整記錄，但不會自動切換到技術輸出；失敗或使用者主動查看時才開啟。

### chore
- **AI Rules Manager v0.2.2** — Extension manifest、lockfile、README 與 VSIX 打包版本準備對齊 `v0.2.2` / `ai-rules-manager-0.2.2.vsix`。

## [2026-07-18] AI Rules Manager v0.2.1

### fix
- **巡檢介面收斂** — 移除 Doctor 與同步覆蓋檢查的指令／面板操作；巡檢介面收斂為 Git-only 狀態。

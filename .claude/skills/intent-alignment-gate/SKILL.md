...
### 1.1 Product Decision Boundary And Overreach Classification

Confirm that the proposed implementation directly serves the user's current
explicit product result. Users decide product goals, behavior, and policies;
AI selects only the implementation method inside the authorized scope. Before
a recommendation becomes an implementation action, use the existing
`overreach_check` classification from `requirement-precision.md` and the
execution-spec contract. Best practice, safety advice, risk discovery, and
possible future needs are not implementation authorization.

Only `explicit-scope` and a qualifying `minimal-implementation-detail` may
continue without a new decision. Classification describes content only; retain
the existing overreach result and routing as the disposition. A
`suggestion-only` item is not implemented, does not block or ask by default,
and leaves the original authorized work running; mention it only if material
to the final result. Ask for an `approval-required-expansion` only when the
current explicit result depends on the product decision, and stop only the
affected action while other authorized work continues. A material risk finding
is a reason to stop or ask about the affected action, not a reason to silently
add product behavior or policy.

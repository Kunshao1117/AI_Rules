...
function Add-ReportSection { param([string]$C, [string]$F) Add-Content $F $C -Encoding UTF8 }
    ...
    try {
        ...
        foreach ($p in $patterns) {
            if (Test-Path "src") {
                ...
                if ($m) { $found = $true; $m | ForEach-Object { Add-ReportSection "  - $($_.Filename):$($_.LineNumber)" $out } }
            }
        }
        if (-not $found) { Add-ReportSection "✅ 未偵測到明顯硬編碼憑證模式" $out }

        Add-ReportSection "`n## 環境變數一致性" $out
        $envEx = Join-Path $ProjectRoot ".env.example"
        if (Test-Path $envEx) {
            $keys = (Get-Content $envEx) | Where-Object { $_ -match "^[A-Z_]+=" } |
                ForEach-Object { ($_ -split "=")[0].Trim() }
            Add-ReportSection ".env.example 定義變數：$($keys.Count) 個" $out
            foreach ($k in $keys) {
                if (Test-Path "src") {
                    $u = Select-String -Path "src/**/*" -Pattern "process\.env\.$k" -Recurse 2>$null
                    if (-not $u) { Add-ReportSection "  🟡 $k — 已定義但未在 src/ 中使用" $out }
                }
            }
        } else { Add-ReportSection "未找到 .env.example 檔案" $out }
...
function Invoke-PerformanceModule {
    ...
    try {
        ...
        if (-not $hasFrontend) {
            Add-ReportSection "本專案無前端頁面，效能掃描跳過。" $out
            Write-Host "⏭️ 無前端頁面，效能掃描跳過"; return
        }
        Add-ReportSection "## Lighthouse 效能掃描" $out
        Add-ReportSection "執行命令（啟動 dev server 後手動執行）：" $out
        Add-ReportSection '```powershell' $out
        Add-ReportSection "npx lighthouse http://localhost:3000 --output=json --output-path=$logsDir/lighthouse.json" $out
        Add-ReportSection '```' $out
        Add-ReportSection "判定標準：Performance < 90 → 🟡（< 70 → 🔴） | LCP > 2.5s → 🟡（> 4s → 🔴）" $out
    } finally { Pop-Location }

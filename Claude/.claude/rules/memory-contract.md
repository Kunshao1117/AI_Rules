...
| **專案記憶卡** | `<project>/.claude/agents/memory/` | 程式碼架構知識、模組決策、追蹤檔案 | Master Agent 透過 Write/Edit |

- **Claude Code 原生記憶體** 使用 `user/feedback/project/reference` 四種類型，儲存跨對話的持久偏好與背景。
- **專案記憶卡**（`.claude/agents/memory/*/SKILL.md`）追蹤程式碼模組的架構決策、已知問題、教訓。
...
│   └── YES → Find or create a matching .claude/agents/memory/ card.
...
- **Directory**: `.claude/agents/memory/` with nested `SKILL.md` files (max 4 levels deep).
...
- If a module is deleted from codebase, physically delete its `.claude/agents/memory/` card. No abandoned cards.

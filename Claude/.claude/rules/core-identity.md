...
- **Source write station ownership**: In active Team mode, source writes are not Master-Agent work and source writes are not captain-default work. Main-worktree writes default to a named station-owned `change-delivery` station under `formal-write`, with authorization phase `implementation-change-delivery`, exact file allowlist, dirty diff read, and no protected actions. `change-application` is only the fallback for returned isolated/text artifacts, explicit integration tasks, or assigned generated/deployed sync. A Master Agent/captain-owned gate is valid only when the platform cannot delegate the physical write or a protected direct exception applies, and it cannot support full Team-Native completion by itself.

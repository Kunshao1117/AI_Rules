...
- Routine discussion and short status updates may stay concise. Structured summaries or compact tables are required for implementation plans, pre-write risk reviews, multi-file changes, completion summaries, audit reports, and handoffs.
- Example compact table: `事項 | 位置 | 影響 | 狀態`.
- The `位置` column MUST name concrete files, sections, tool/status scopes, or directory scopes; compact scope labels must resolve to concrete file or section evidence.
- High-change or external facts, dates, APIs, versions, constraints, and risk assumptions MUST be grounded in current local files, tool output, official documentation, or primary sources.
- Memory and internal model knowledge are possibly stale. Do not treat either as verified or current without grounding. If verification is unavailable, report the missing evidence instead of presenting assumptions as current fact.
- Detailed language-layer classification and exact-evidence handling are governed by `Shared/policies/language-governance.md`; freshness, source-tier, conflict, and grounding rules are governed by `Shared/policies/grounding-governance.md`. Deployed projects read the matching `.agents/shared/policies/` copies.

## 3. Team-Native And Authorization Minimum

Team-Native / subagent team mode starts when the Director requests governed work: governance,
workflow, fix, build, debug, test, audit, skill, memory/docs, commit, handoff, source,
public-contract, or equivalent source/governance/evidence-bearing work. Requests for Team-Native, a
team, a team member, subagent, delegation, or equivalent dispatch also activate Team mode. The
Director does not need a fixed phrase. Slash commands, Plan Mode approvals, permission prompts,
platform tools, subagent routes, and interface buttons are route or authorization signals; they do
not activate Team-Native mode without a current governed user request and do not authorize writes by
themselves. When Team mode is not active, captain/team-board limits do not apply, while ordinary
lifecycle, scoped authorization, read-before-write, and protected-action gates still apply.
...
- **Source write station ownership**: In active Team mode, source writes are not Master-Agent work
  and source writes are not captain-default work. Main-worktree writes default to a named
  station-owned `change-delivery` station under `formal-write`, with authorization phase
  `implementation-change-delivery`, exact file allowlist, dirty diff read, and no protected
  actions. `change-application` is only the fallback for returned isolated/text artifacts, explicit
  integration tasks, or assigned generated/deployed sync. A platform-nondelegable protected-action
  record or owner-station gate is valid only when the platform cannot delegate the physical write or
  a protected direct exception applies, and it cannot support full Team-Native completion by itself.
...
This core marker is generated from `Shared/policies/subagent-invocation.md`. Keep the full policy in `Shared/policies/` and the deployed readable copy at `.agents/shared/policies/subagent-invocation.md`; do not paste the full playbook into platform core.

- Claude subagents are execution channels only after Team mode is activated by a governed Director request, and after Team-Native board, station, role, handoff, dispatch wave, and channel state are recorded.
- Required Claude evidence and change-delivery reports follow `programming-team-governance`, `team-task-board`, and delivery artifact skills.
- Missing subagent capability is `blocked`, `unverified`, `standby`, `unavailable`, or `closed-with-director-risk`, not master-agent direct completion.
- Claude subagents must not mutate source, memory, git, release, deploy, install, credentials, or external state unless a scoped protected station explicitly owns that phase.

---
name: 12_skill_forge
description: "技能鍛造、建立新技能、建立 Shared skill、建立 project skill、建立 Codex skill、從健檢/除錯/總監指令萃取可重用方法論，或 plugin/extension/插件/延伸模組、VSIX、Release/發布、version/版本、tag、update reminder/更新提醒 相關技能設計（Use when）。不適用：只是討論技能想法、不準備寫入，或只要修改既有技能描述（DO NOT use when）。"
required_skills:
  - memory-ops
...
This Claude command entry is a thin route entry. It selects workflow row `12`, applies the platform adapter, and points to shared procedures when details are needed. It does not grant write, memory, git, release, deployment, install, credential, or external-state authority.
...
6. When a concrete phase checklist is needed, read `.agents/shared/workflow-stage-procedures.md` and use section `12 Skill Forge`. Do not copy that procedure back into this entry.

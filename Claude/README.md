...
08 以外的工作流共用外部接地矩陣：每個 Slash Command 都對應任務型態、官方或標準依據、最低證據狀態、阻塞條件與下一流程路由。Claude 版可把矩陣轉譯成計畫模式、子代理、權限、鉤子、檢查點與批次命令證據，但不得讓這些能力取代主代理的 GO gate 與最終整合責任；必要證據分支或任務板不可用時必須標記未驗證、阻塞或總監風險關閉但非完整。

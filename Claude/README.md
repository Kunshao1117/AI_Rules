...
4. **技能重複維護** — 兩個 AI 的技能各自維護 → 43 套操作型技能與 Gemini 版完全同步
...
| **隊長制編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入隊長制；每站必須標示是否適用、執行模式、證據負責人、角色邊界、主線直做例外與完成條件；證據型站點預設使用 Claude 子代理或等價唯讀證據分支，實作隊員只能在受治理隔離區產出 patch packet，不能取代主代理寫檔、審查狀態或交付裁決 |
...
graph TB
    ...
    subgraph ".claude/ 生態系統（部署後）"
        ...
        SKILLS[".claude/skills/<br/>43 套操作型技能"]
    end
...
| **Captain-led programming governance coverage** | Doctor 檢查隊長制編程治理共用技能、隊長任務板、角色邊界、隔離補丁、證據負責人、主線直做例外、全主線假團隊防線、00/01 自動轉向、Claude command 接入與部署後 shared skill / shared reference hash 是否一致 |
...
1. **專職化分工** — 主代理人直接執行與裁決；下游 `.agents/shared/policies/subagent-invocation.md` 與框架來源 `Shared/policies/subagent-invocation.md` 只定義 Delegation Gate、角色互斥、隔離補丁、假團隊防線與主線直做例外；編程工作先過隊長任務板，Claude adapter 把證據型站點轉成 description-driven subagent、`@agent` 或受控 `Agent(...)`，實作型隊員只能產出隔離補丁，不能自我審查；不可用時標記未驗證、阻塞或具體主線直做例外
...
08 以外的工作流共用外部接地矩陣：每個 Slash Command 都對應任務型態、官方或標準依據、最低證據狀態、阻塞條件與下一流程路由。Claude 版可把矩陣轉譯成計畫模式、子代理、權限、鉤子、檢查點與批次命令證據，但不得讓這些能力取代主代理的 GO gate 與最終整合責任；必要證據分支不可用時必須標記未驗證、阻塞或具體主線直做例外。
...
| **子代理人** | 隊長站點 → Gemini CLI / `@` 指派 / browser-capable agent / Antigravity plugin adapter / 受治理隔離補丁 | 隊長站點 → description-driven subagent / `@agent` / governed `Agent(...)` / 受治理隔離補丁；不可用時標記未驗證、阻塞或主線直做例外 |
...
| **操作型技能** | `.agents/skills/` (43 個) | `.claude/skills/` (43 個) |
...
└── .claude/
    ...
    ├── skills/                  ← 操作型知識庫（43 個，部署時從 Shared/ 注入）
    │   ├── _index.md            ← 技能索引
    │   ├── memory-ops/
    │   ├── memory-arch/
    │   ├── code-quality/

...
| `/08_audit` | 專案型態偵測、平台能力快照、動態掛載模組與證據式健康報告 | Reader/Logs |
...
Claude 健檢入口採全光譜證據式架構：先偵測網站、後端、命令列、桌面、外掛、函式庫、基礎設施、資料管線、AI 功能、文件治理庫或混合專案，再依平台能力掛載基礎、治理、邏輯、真實操作、效能、發布與相容性檢查。Claude 版優先使用子代理、鉤子、權限模式、檢查點、批次讀取與非互動命令採證；中繼證據只允許寫入健檢日誌，不授權修改原始碼、記憶卡或外部狀態。
...
| | `audit-engine` | 全光譜健檢語義引擎（專案型態、證據包、燈號規則、安全/API/資料流與相容性） | — |

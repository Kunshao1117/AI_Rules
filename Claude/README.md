...
4. **技能重複維護** — 兩個 AI 的技能各自維護 → 44 套操作型技能與 Gemini 版完全同步
...
| **隊長制編程團隊治理** | 開發、修改、修復、測試、除錯、健檢、提交、交接與技能鍛造會依語意自動進入隊長制；總監只對隊長說話，隊長派一任務一隊員；證據型站點預設使用 Claude 子代理或等價唯讀證據分支，實作隊員只能在受治理隔離區產出 patch packet，或在無隔離時產出文字補丁任務包；審查與驗證由不同隊員互驗，隊長最後只整合已回收且可追溯的補丁與證據，不能讓隊員取代主代理寫檔、審查狀態或交付裁決 |
...
graph TB
    ...
    subgraph ".claude/ 生態系統（部署後）"
        ...
        SKILLS[".claude/skills/<br/>44 套操作型技能"]
    end
...
| **Captain-led programming governance coverage** | Doctor 檢查隊長制編程治理共用技能、團隊任務包模板、隊長任務板、角色邊界、隔離補丁、文字補丁、證據負責人、主線直做例外、全主線假團隊防線、00/01 自動轉向、Claude command 接入與部署後 shared skill / shared reference hash 是否一致 |
...
| `/03_build` | 兩階段建構：Plan Mode 計畫 → GO → 派實作補丁包 → 審查/驗證包互驗 → 隊長整合 → 記憶歸卡與真實執行驗證 | Captain/SRE |
| `/03-1_experiment` | 沙盒快速實驗（保留最小團隊站點宣告） | Experiment Worker |
| `/04_fix` | 兩階段修復：診斷計畫 → GO → 派修復補丁包 → 審查/回歸驗證包互驗 → 隊長整合 → 記憶更新與真實失敗路徑回歸 | Captain/SRE |
...
| `/09_commit` | 授權備份：掃描 → CHANGELOG 草稿 → GO → 審查/收尾證據包 → 隊長更新變更紀錄 + 明確清單遠端備份 | Captain/SRE |
...
| **操作型技能** | `.agents/skills/` (44 個) | `.claude/skills/` (44 個) |
...
└── .claude/
    ...
    ├── skills/                  ← 操作型知識庫（44 個，部署時從 Shared/ 注入）
    │   ├── _index.md            ← 技能索引
    │   ├── memory-ops/
    │   ├── memory-arch/
    │   ├── code-quality/

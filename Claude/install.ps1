...
<#
.SYNOPSIS
    Antigravity Claude Edition — 遠端安裝啟動器

.DESCRIPTION
    從 GitHub 下載並部署 Antigravity Claude Edition 框架到指定專案目錄。
...
.EXAMPLE
    ...
    $repo = 'AI_Rules'
    $ref = 'main'
    $platformPath = 'Claude/install.ps1'
    $expectedInstallSha256 = ''
    $installerArgs = @()
    if ($ref -notmatch '^[A-Za-z0-9._-]+$' -or $ref.Contains('..')) { throw "Unsafe Claude install ref: $ref" }
    $u = "https://raw.githubusercontent.com/$owner/$repo/$ref/$platformPath"
    $uri = [Uri]$u
    if ($uri.Scheme -ne 'https' -or $uri.Host -ne 'raw.githubusercontent.com' -or $uri.AbsolutePath -ne "/$owner/$repo/$ref/$platformPath") { throw "Unexpected Claude install source: $u" }
    $f = Join-Path $env:TEMP 'cc_install.ps1'
    $receipt = Join-Path $env:TEMP 'cc_install.receipt.json'
    try {
        ...
        if ($expectedInstallSha256 -and $actualInstallSha256 -ne $expectedInstallSha256.ToLowerInvariant()) { throw "Claude install script SHA256 mismatch." }
...
$receiptPath = if ([string]::IsNullOrWhiteSpace($DownloadReceipt)) {
    Join-Path $env:TEMP "claude_edition_receipt_$([Guid]::NewGuid().ToString('N')).json"
} else {
...
$tempName = "claude_edition_$([Guid]::NewGuid().ToString('N'))"
$tempZip = Join-Path $env:TEMP "$tempName.zip"
$tempDir = Join-Path $env:TEMP $tempName

try {
    Write-Host ""
    Write-Host "[>] Antigravity Claude Edition 框架安裝程式"
    ...
    if ($RemoveOrphans) {
        & $deployScript -Platform Claude -Mode $Mode -Target $targetPath -RemoveOrphans
    } else {
        & $deployScript -Platform Claude -Mode $Mode -Target $targetPath
    }
